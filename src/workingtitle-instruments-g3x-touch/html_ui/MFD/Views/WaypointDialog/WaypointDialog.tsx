import {
  ArrayUtils, DebounceTimer, Facility, FacilitySearchType, FacilityType, FacilityWaypoint, FSComponent, ICAO,
  IcaoValue, IntersectionFacilityUtils, MappedSubject, NodeReference, Subject, VNode
...
import { G3XFacilityUtils } from '../../../Shared/Navigation/G3XFacilityUtils';
    ...
    if (this.facilitySearchType === FacilitySearchType.AllExceptVisual || this.facilitySearchType === FacilitySearchType.User) {
      ...
      allMatches = allMatches.filter(icao => {
        return !ICAO.isValueFacility(icao, FacilityType.USR)
          || icao.airport === G3XFacilityUtils.USER_FACILITY_SCOPE;
      });
    }
    ...
    if (exactMatches.length > 0) {
      const exactMatchesWithFacilities = (await Promise.all(
        ...
        IntersectionFacilityUtils.filterDuplicates(exactMatches)
          .map<Promise<SearchResult | null>>(async match => {
