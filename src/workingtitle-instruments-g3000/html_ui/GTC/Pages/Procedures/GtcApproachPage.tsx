...
import { GtcControlMode, GtcService, GtcViewEntry, GtcViewLifecyclePolicy } from '../../GtcService/GtcService';
...
export class GtcApproachPage extends GtcProcedureSelectionPage<GtcApproachPageProps> {
   ...
   */
  private async onLoadButtonPressed(activate: boolean): Promise<void> {
    ...
    if (!facility || !approach || !transition) {
      return;
    }
    ...
    if (!approach.isVisualApproach) {
      ...
      const accepted = await GtcDialogs.openMessageDialog(this.gtcService, 'Obstacle clearance is not provided for visual approaches.');
    ...
    if (success) {
      ...
      (activeViewEntry as GtcViewEntry<GtcFlightPlanPage>).ref.scrollTo(FlightPlanSegmentType.Approach);
    }
  }
