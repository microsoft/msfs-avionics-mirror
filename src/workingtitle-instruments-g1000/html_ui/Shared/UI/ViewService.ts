import { EventBus, FSComponent, HEvent, NodeReference, Subject, Subscribable, Subscription, VNode } from '@microsoft/msfs-sdk';
...
export abstract class ViewService {
  ...
  public open<T extends UiView = UiView>(type: string, isSubView = false): T {
    ...
    view.onClose.clear();
    viewEntry.onCloseSub = view.onClose.on(this.viewClosedHandler);
  ...
  private handleViewClosed(closedView: UiView): void {
    ...
    if (viewIndex > -1) {
      const viewEntry = this.openViews[viewIndex];
      viewEntry.onCloseSub?.destroy();
      viewEntry.onCloseSub = undefined;
