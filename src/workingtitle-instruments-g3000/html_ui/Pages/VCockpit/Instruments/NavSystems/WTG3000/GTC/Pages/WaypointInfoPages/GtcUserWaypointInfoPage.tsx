import {
  ArraySubject, BasicNavAngleSubject, BasicNavAngleUnit, ComponentProps, DisplayComponent, Facility, FacilityLoader, FacilitySearchType,
  FacilityType, FacilityUtils, FacilityWaypoint, FSComponent, GeoPoint, ICAO, MagVar, MappedSubject, MutableSubscribable, NodeReference,
  NumberFormatter, NumberUnitSubject, SetSubject, Subject, Subscribable, SubscribableArray, SubscribableArrayEventType, Subscription, Unit, UnitFamily,
  UnitType, UserFacility, UserFacilityType, VNode
...
export class GtcUserWaypointInfoPage extends GtcWaypointInfoPage<FacilitySearchType.User, GtcUserWaypointInfoPageProps> {
   ...
   */
  public async initSelection(facility?: UserFacility | string): Promise<void> {
    let selection: FacilityWaypoint<UserFacility> | null = null;
    if (facility === undefined) {
      selection = this.props.selectedUserWaypoint.get();
    } else {
      if (typeof facility === 'string') {
        if (ICAO.isFacility(facility, FacilityType.USR)) {
          try {
            facility = await this.props.facLoader.getFacility(FacilityType.USR, facility);
    ...
    if (selection !== null && !this.props.controller.doesUserWaypointExist(selection.facility.get().icao)) {
