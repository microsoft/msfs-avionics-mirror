...
/**
 * A utility class for working with WT21 autopilots.
 */
export class WT21APUtils {
  /**
  ...
  public static navInterceptCurve(distanceToSource: number, deflection: number, xtk: number, tas: number, isLoc?: boolean): number {
    if (isLoc) {
      return WT21APUtils.localizerInterceptCurve(deflection, xtk, tas);
    } else {
      return WT21APUtils.defaultInterceptCurve(xtk, tas);
    }
  }
  ...
  public static localizerInterceptCurve(deflection: number, xtk: number, tas: number): number {
    ...
    const interceptAngle = WT21APUtils.calculateTurnBasedInterceptAngle(turnRadiusMeters, xtkMeters);
