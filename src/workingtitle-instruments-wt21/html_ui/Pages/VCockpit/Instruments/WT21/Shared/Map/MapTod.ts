import {
  ...
  AbstractMapWaypointIconOptions, BasicFacilityWaypoint, ClippedPathStream, EventBus, Facility, GeoPoint, ImageCache,
  MapCullableLocationTextLabel, MapCullableTextLabel, MapProjection, MapWaypointIcon, MapWaypointImageIcon,
  MapWaypointRendererIconFactory, MapWaypointRendererLabelFactory, ReadonlyFloat64Array, UnitType, Vec2Math, VNavWaypoint, Waypoint
...
export class MapFixInfoWaypointIcon<T extends Waypoint> extends AbstractMapWaypointIcon<T> {
  ...
  private drawDistanceCircle(
    ...
    // this code from the Boeing does not work well
    // const gaRadiansToPixels = mapProjection.getGeoProjection().getScaleFactor();
    // const radiusPixels = gaRadians * gaRadiansToPixels;

    // Apply a different technique and just project a point on the radius and figure out it's distance in pixels.
