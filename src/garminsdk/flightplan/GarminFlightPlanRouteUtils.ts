import {
  ...
  FlightPlanLeg, FlightPlanRoute, FlightPlanRouteEnrouteLeg, FlightPlanRouteUtils, FlightPlanSegmentType, ICAO,
  LegDefinitionFlags, LegType, RunwayUtils
} from '@microsoft/msfs-sdk';

/**
 * A utility class for working with Garmin flight plan routes.
 */
export class GarminFlightPlanRouteUtils {
  ...
  public static async createRouteFromFlightPlan(
    ...
    const originAirport = flightPlan.originAirportIcao !== undefined && ICAO.isValueFacility(flightPlan.originAirportIcao, FacilityType.Airport)
    ...
    const destinationAirport = flightPlan.destinationAirportIcao !== undefined && ICAO.isValueFacility(flightPlan.destinationAirportIcao, FacilityType.Airport)
      ? await facLoader.getFacility(
        FacilityType.Airport,
    ...
    for (const segment of flightPlan.segmentsOfType(FlightPlanSegmentType.Enroute)) {
      if (segment.airway) {
        // segment.airway is of the format `${airwayName}.${exitWaypointIdent}`.
        const airwayName = segment.airway.split('.')[0];
        const exit = ArrayUtils.peekLast(segment.legs);
        if (exit && airwayName.length < 8) {
          const enrouteLeg = FlightPlanRouteUtils.emptyEnrouteLeg();
          enrouteLeg.fixIcao = exit.leg.fixIcaoStruct;
          enrouteLeg.via = airwayName;
