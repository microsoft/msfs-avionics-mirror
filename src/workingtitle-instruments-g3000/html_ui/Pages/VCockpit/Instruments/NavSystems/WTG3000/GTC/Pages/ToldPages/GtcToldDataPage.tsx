import {
  AirportFacility, FacilityLoader, FacilityType, FSComponent, ICAO, MagVar, MappedSubject, NodeReference, OneWayRunway,
  RunwayUtils, Subject, Subscribable, Subscription, UserSetting, VNode
} from '@microsoft/msfs-sdk';
import { AirportWaypoint, GarminFacilityWaypointCache, UnitsUserSettings } from '@microsoft/msfs-garminsdk';
import {
  FlightPlanStore, PerformanceConfig, ToldLandingPerformanceResult, ToldTakeoffPerformanceResult, ToldUserSettings, VSpeedGroup,
  VSpeedGroupType, WeightFuelUserSettings
...
> extends GtcView<P> {
   ...
   * @param icao The origin/destination ICAO.
   */
  private async onOriginDestIcaoChanged(icao: string): Promise<void> {
    ...
    if (selectedAirport?.facility.get().icao !== airportIcao) {
      const opId = ++this.originDestOpId;

      try {
        airportFacility = await this.props.facLoader.getFacility(FacilityType.Airport, airportIcao);
        ...
        console.warn(`GtcTakeoffDataPage: could not retrieve airport facility for ICAO: ${icao}`);
  ...
  private updateOriginDestIcao(): void {
    ...
    if (selectedRunway === null) {
      this.originDestIcaoSetting.value = selectedAirport.facility.get().icao;
    } else {
      this.originDestIcaoSetting.value = RunwayUtils.getRunwayFacilityIcao(selectedAirport.facility.get(), selectedRunway);
    }
  }
