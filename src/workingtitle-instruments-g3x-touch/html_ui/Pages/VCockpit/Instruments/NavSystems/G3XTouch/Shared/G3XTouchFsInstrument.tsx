import {
  AccelerometerPublisher, AdcPublisher, AhrsPublisher, AutopilotInstrument, AvionicsSystem,
  ...
  FlightPathCalculator, FlightPlanner, FlightTimerPublisher, FSComponent, FsInstrument, GameStateProvider,
  GNSSPublisher, GPSSatComputer, HEventPublisher, InstrumentBackplane, LNavObsSimVarPublisher, LNavSimVarPublisher,
...
import {
  ...
  GarminSpeedConstraintStore, GarminVNavUtils, GpsNavSource, GpsReceiverSelector,
  GpsReceiverSystem, LNavDataSimVarPublisher, MagnetometerSystem, MarkerBeaconSystem, NavDataFieldGpsValidity,
  NavEventsPublisher, NavRadioNavSource, NavReferenceIndicatorsCollection, NavReferenceSource,
  NavReferenceSourceCollection, NullNavSource, RadarAltimeterSystem
  ...
  protected createSystems(): void {
    ...
    for (let index = 1; index <= this.config.sensors.gpsReceiverDefinitions.length; index++) {
      ...
      const gpsSatComputer = this.gpsSatComputers[index] = new GPSSatComputer(
        index,
        this.bus,
        'coui://html_ui/Pages/VCockpit/Instruments/NavSystems/G3XTouch/Assets/Data/gps_ephemeris.json',
        'coui://html_ui/Pages/VCockpit/Instruments/NavSystems/G3XTouch/Assets/Data/gps_sbas.json',
        5000,
    ...
    this.uiService.registerMfdView(UiViewStackLayer.Overlay, UiViewLifecyclePolicy.Transient, UiViewKeys.PfdOptions, (uiService, containerRef) => {
      return (
        <PfdOptionsView
          ...
          externalFplSourceCount={this.config.fms.externalFplSourceCount}
          radiosConfig={this.config.radios}
        />
      );
    });
