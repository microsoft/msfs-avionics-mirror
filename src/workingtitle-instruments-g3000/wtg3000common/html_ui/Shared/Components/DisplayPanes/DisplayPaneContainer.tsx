...
export class DisplayPaneContainer extends DisplayComponent<DisplayPaneContainerProps> {
   ...
   * @throws Error if this container has been destroyed.
   */
  public routeInteractionEvent(pane: 'left' | 'right', event: string): boolean {
    if (!this.isAlive) {
      throw new Error('DisplayPaneContainer: cannot invoke a dead container');
    }
    ...
    if (pane === 'left') {
      return this.leftPaneRef.getOrDefault()?.onInteractionEvent(event) ?? false;
    } else {
      return this.rightPaneRef.getOrDefault()?.onInteractionEvent(event) ?? false;
    }
  }
