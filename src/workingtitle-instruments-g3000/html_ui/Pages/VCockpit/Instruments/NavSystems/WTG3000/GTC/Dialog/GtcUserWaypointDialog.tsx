...
export type GtcUserWaypointDialogEditInput = {
  ...
  editFacility: string | UserFacility;
}
...
export class GtcUserWaypointDialog extends GtcView<GtcUserWaypointDialogProps> implements GtcDialogView<GtcUserWaypointDialogInput, UserFacility> {
   ...
   */
  private async initEditing(fac: string | UserFacility): Promise<void> {
    if (typeof fac === 'string') {
      if (!ICAO.isFacility(fac, FacilityType.USR)) {
        return;
      }
  ...
  private async tryCreateWaypoint(): Promise<void> {
    ...
    const icao = `U      ${this.store.ident.get()}`;
    ...
    if (this.props.controller.doesUserWaypointExist(icao) && icao !== editingFacility?.icao) {
    ...
    if (editingFacility !== null) {
      ...
      const flightPlanStatus = this.props.controller.getWaypointFlightPlanStatus(editingFacility.icao);
      ...
      this.props.controller.editUserFacility(editingFacility.icao, facility);
    } else {
  ...
  private async selectIdent(): Promise<void> {
    const result = await this.props.gtcService.openPopup<GtcKeyboardDialog<string>>(GtcViewKeys.KeyboardDialog, 'normal', 'hide')
      .ref.request({
        ...
        <div class='usr-wpt-dialog-top-row'>
          ...
          >
            <div class='usr-wpt-dialog-ident-button-container'>
              ...
              <img src='coui://html_ui/Pages/VCockpit/Instruments/NavSystems/WTG3000/Assets/Images/GTC/icon_small_user.png' class='usr-wpt-dialog-ident-button-icon' />
            </div>
          </GtcTouchButton>
