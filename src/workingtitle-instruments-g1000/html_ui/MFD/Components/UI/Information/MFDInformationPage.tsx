import {
  CompiledMapSystem, ControlPublisher, EventBus, FacilityLoader, FocusPosition, FSComponent, ICAO, MapIndexedRangeModule,
  MapSystemBuilder, MathUtils, Subject, Vec2Math, VecNMath, VNode, Waypoint
} from '@microsoft/msfs-sdk';

import {
  AirportWaypoint, Fms, GarminMapKeys, MapPointerController, MapPointerInfoLayerSize, MapPointerModule, MapRangeController,
  UnitsUserSettings, WaypointMapRTRController, WaypointMapSelectionModule
...
export abstract class MFDInformationPage extends MFDUiPage<MFDInformationPageProps> {
  ...
  private readonly mapWaypointRTRController = this.compiledMap.context.getController(GarminMapKeys.WaypointRTR);
  ...
  public onAfterRender(thisNode: VNode): void {
    ...
    this.waypoint.pipe(this.mapWptSelectionModule.waypoint);
  }
  ...
  protected onViewClosed(): void {
    ...
    if (this.waypoint.get() instanceof AirportWaypoint) {
      this.mapWaypointRTRController.tryTargetWaypoint(true);
    } else {
      this.mapRangeController.setRangeIndex(this.getDefaultRangeIndex());
    }
  }
