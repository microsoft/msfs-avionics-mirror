...
import { Fms, DirectToState, FmsUtils } from '@microsoft/msfs-garminsdk';
...
export class FlightPlanAsoboSync {
  ...
  public static async loadFromGame(fms: Fms): Promise<void> {
    ...
    if (!isDirectTo) {
      ...
      lastEnrouteSegmentIndex = FlightPlanAsoboSync.setEnroute(data, plan, fms);
      ...
      if (destFacilityType !== FacilityType.Airport && destFacilityType !== undefined) {
        FlightPlanAsoboSync.buildNonAirportDestLeg(data, plan, fms, lastEnrouteSegmentIndex);
      }

      // check end of enroute for duplicate
      const lastEnrouteSegment = plan.getSegment(lastEnrouteSegmentIndex);
      if (lastEnrouteSegment.legs.length > 0) {
        const lastEnrouteLegGlobalIndex = FmsUtils.getGlobalLegIndex(plan, lastEnrouteSegmentIndex, lastEnrouteSegment.legs.length - 1);
        const lastEnrouteLeg = plan.tryGetLeg(lastEnrouteLegGlobalIndex);
        const nextLeg = plan.tryGetLeg(lastEnrouteLegGlobalIndex + 1);
        if (lastEnrouteLeg !== null && nextLeg !== null && ICAO.getIdent(lastEnrouteLeg.leg.fixIcao) === ICAO.getIdent(nextLeg.leg.fixIcao)) {
          fms.removeWaypoint(lastEnrouteSegmentIndex, lastEnrouteSegment.legs.length - 1);
        }
      }
  ...
  public static async SaveToGame(fms: Fms): Promise<void> {
    ...
    if (fms.getDirectToState() === DirectToState.TORANDOM) {
      return;
    }
   ...
   * @param fms an instance of the fms
   * @returns the last enroute segment index
   */
