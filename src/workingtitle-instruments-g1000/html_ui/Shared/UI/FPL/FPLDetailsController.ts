...
import {
  ...
  LegEventType, NodeReference, OriginDestChangeType, SegmentEventType, SubscribableArrayEventType, UnitType, VNavEvents, VNavPathCalculator, VNavUtils
} from '@microsoft/msfs-sdk';

import { DirectToState, Fms, FmsUtils } from '@microsoft/msfs-garminsdk';
...
export class FPLDetailsController {
  ...
  public initialize(): void {
    ...
    fpl.on('fplSegmentChange').handle((event) => {
      ...
      if (event.type !== SegmentEventType.Changed && event.segmentIndex <= this.store.activeLeg.get().segmentIndex) {
    ...
    fpl.on('fplLegChange').handle((event) => {
      ...
      if (
        event.type !== LegEventType.Changed
        && (
    ...
    if (this.fms.verticalPathCalculator !== undefined) {
      this.fms.verticalPathCalculator.vnavCalculated.on((sender: VNavPathCalculator, planIndex: number) => {
        this.onVnavUpdated(planIndex);
      });
    }
  ...
  private onVnavUpdated(planIndex: number): void {
    if (this.hasVnav && this.fms.verticalPathCalculator !== undefined && planIndex === Fms.PRIMARY_PLAN_INDEX) {
      ...
      if (this.fms.getDirectToState() === DirectToState.TOEXISTING) {
        ...
        const lateralLeg = lateralPlan.tryGetLeg(lateralPlan.directToData.segmentIndex, vnavLeg.legIndex);
        section?.instance.setIsUserConstraint(lateralPlan.directToData.segmentLegIndex, lateralLeg !== null ? FmsUtils.isLegAltitudeEdited(lateralLeg, false) : false);
      }
    }
  }
  ...
  private onFlightPlanLoaded(e: FlightPlanIndicationEvent): void {
    ...
    for (let i = 0; i < plan.segmentCount; i++) {
      ...
      this.onSegmentChange({ planIndex: e.planIndex, segmentIndex: i, segment: segment, type: SegmentEventType.Added });
