import {
  AdcEvents, AirportFacility, APVerticalModes, AvionicsSystemState, AvionicsSystemStateEvent, ClockEvents, ConsumerSubject,
  ConsumerValue, EventBus, FacilityLoader, GeoPoint, GeoPointInterface, GNSSEvents, MappedSubject, MultiExpSmoother, NearestAirportSubscription,
  ObjectSubject, OneWayRunway, RunwaySurfaceType, RunwayUtils, Subject, Subscription, UnitType
...
export class Gpws {
  ...
  private readonly fmaData = ConsumerSubject.create<APVerticalModes | undefined>(null, undefined);
  ...
  public init(): void {
    ...
    const sub = this.bus.getSubscriber<ClockEvents & RASystemEvents & AdcEvents & GNSSEvents & FmaData>();

    this.simRate.setConsumer(sub.on('simRate'));

    this.isOnGround.setConsumer(sub.on('on_ground'));

    this.radarAltimeterState.setConsumer(sub.on('ra_state'));
    this.radarAltitudeSource.setConsumer(sub.on('ra_radio_alt'));

    this.fmaData.setConsumer(sub.on('verticalActive'));
    ...
    this.gpsPosSub = sub.on('gps-position').handle(updateGpsPos);
