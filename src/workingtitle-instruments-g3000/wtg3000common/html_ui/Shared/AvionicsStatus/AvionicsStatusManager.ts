...
import { AvionicsStatusUtils } from './AvionicsStatusUtils';

/**
 * A status update from an {@link AvionicsStatusClient}.
 */
type AvionicsStatusClientSyncData = {
  /** The instrument type of the client's avionics unit. */
...
export class AvionicsStatusClient {
  ...
  private sendSyncData(isInitial: boolean): void {
    ...
    this.syncPublisher.pub(
      ...
      { instrumentType: this.instrumentType, instrumentIndex: this.instrumentIndex, status: this.status, isInitial },
      true,
      false
    );
  }
