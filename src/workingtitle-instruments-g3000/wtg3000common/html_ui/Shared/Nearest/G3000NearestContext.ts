import {
  AdaptiveNearestContext, AdaptiveNearestSubscription, AirportFacility, AirportFacilityDataFlags, BitFlags,
  ConsumerSubject, EventBus, FacilityFrequencyType, FacilityLoader, FacilityType, FacilityTypeMap, GeoPoint,
  GeoPointSubject, IntersectionFacility, IntersectionType, MappedSubject, NdbFacility, NearestAirportSubscription,
  NearestSubscription, RadioUtils, ReadonlySubEvent, RunwayUtils, SubEvent, Subscribable, SubscribableUtils,
  Subscription, UnitType, UserFacility, VorClass, VorFacility, VorType
...
export class G3000NearestContext {
  ...
  private readonly _weather: AdaptiveNearestSubscription<NearestAirportSubscription>;
  ...
  private constructor(
    ...
    this._weather = new AdaptiveNearestSubscription(new NearestAirportSubscription(facilityLoader), 200);
    this._weather.start();
  ...
  private async initFilters(): Promise<void> {
    ...
    this.context.airports.innerSubscription.setDataFlags(
    ...
    this._weather.innerSubscription.setDataFlags(
      AirportFacilityDataFlags.Departures
      | AirportFacilityDataFlags.Arrivals
      | AirportFacilityDataFlags.Approaches
      | AirportFacilityDataFlags.Frequencies
    ...
    this.context.usrs.innerSubscription.setFacilityFilter(facility => {
      return facility.icaoStruct.airport === G3000FacilityUtils.USER_FACILITY_SCOPE;
    });
  }
