import { DisplayComponent, FSComponent, MutableSubscribable, Subscribable, SubscribableSet, ToggleableClassNameRecord, VNode } from '@microsoft/msfs-sdk';

import { ToggleTouchButtonProps, TouchButtonHoldAction, TouchButtonHoldEndReason, TouchButtonOnTouchedAction } from '@microsoft/msfs-garminsdk';
 ...
 */
export interface UiToggleTouchButtonProps<S extends Subscribable<boolean> | MutableSubscribable<boolean>>
  extends Omit<ToggleTouchButtonProps<S>, 'onTouched' | 'onPressed' | 'onHoldStarted' | 'onHoldTick' | 'onHoldEnded' | 'focusOnDrag' | 'class'> {
   ...
   */
  onHoldTick?: <B extends UiToggleTouchButton<S> = UiToggleTouchButton<S>>(
    button: B,
    state: S,
    dt: number,
    totalTime: number,
   ...
   */
  onHoldEnded?: <B extends UiToggleTouchButton<S> = UiToggleTouchButton<S>>(
    button: B,
    state: S,
 ...
 * focus.
 */
export class UiToggleTouchButton<S extends Subscribable<boolean> | MutableSubscribable<boolean>> extends DisplayComponent<UiToggleTouchButtonProps<S>>
  implements UiFocusableComponent {

  /** @inheritdoc */
  public readonly isUiFocusableComponent = true;

  protected readonly buttonRef = FSComponent.createRef<ToggleTouchButton<S>>();
  ...
  protected onPressed(button: ToggleTouchButton<S>, state: S, isHeld: boolean): void {
    if (this.props.onPressed) {
      this.props.onPressed(this, state, isHeld);
