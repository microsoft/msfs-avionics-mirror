import {
  ...
  GeoPoint, GeoPointSubject, ICAO, LatLonInterface, MagVar, NumberUnitSubject, Subject, Subscribable, Subscription,
...
export class GtcUserWaypointDialogStore {
  ...
  private async syncRadialDistance(fac: UserFacility, opId: number): Promise<void> {
    const referenceIcao = fac.reference1Icao;
    let referenceFac: Facility | null = null;
    let radial: number | undefined;
    let distance: number | undefined;

    if (referenceIcao !== undefined && ICAO.isFacility(referenceIcao)) {
      if (ICAO.isFacility(referenceIcao, FacilityType.USR)) {
        ...
        try {
          referenceFac = await this.facLoader.getFacility(ICAO.getFacilityType(referenceIcao), referenceIcao);
          radial = fac.reference1Radial;
          distance = fac.reference1Distance;
        } catch {
          // noop
  ...
  private async syncRadialRadial(fac: UserFacility, opId: number): Promise<void> {
    const reference1Icao = fac.reference1Icao;
    const reference2Icao = fac.reference2Icao;
    ...
    if (reference1Icao !== undefined && ICAO.isFacility(reference1Icao)) {
      if (ICAO.isFacility(reference1Icao, FacilityType.USR)) {
        ...
        try {
          reference1Fac = await this.facLoader.getFacility(ICAO.getFacilityType(reference1Icao), reference1Icao);
          radial1 = fac.reference1Radial;
        } catch {
          // noop
        }
    ...
    if (reference2Icao !== undefined && ICAO.isFacility(reference2Icao)) {
      if (ICAO.isFacility(reference2Icao, FacilityType.USR)) {
        ...
        try {
          reference2Fac = await this.facLoader.getFacility(ICAO.getFacilityType(reference2Icao), reference2Icao);
          radial2 = fac.reference2Radial;
        } catch {
          // noop
        }
