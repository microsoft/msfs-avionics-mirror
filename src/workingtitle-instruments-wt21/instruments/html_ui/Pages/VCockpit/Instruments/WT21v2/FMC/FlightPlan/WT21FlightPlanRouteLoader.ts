 ...
 */
export class WT21FlightPlanRouteLoader {
  private loadOpId = 0;
   ...
   */
  public constructor(private readonly fms: WT21Fms) {
  }
  ...
  private async loadEnrouteLegs(opId: number, plan: FlightPlan, route: ReadonlyFlightPlanRoute): Promise<LegDefinition | null> {
    ...
    for (const leg of route.enroute) {
      ...
      if (leg.hasLatLon || leg.hasPointBearingDistance) {
        if (!WT21PilotWaypointUtils.isLimitReached(userFacilities)) {
          let facility: UserFacility | undefined;

          if (leg.hasLatLon) {
            const ident = WT21PilotWaypointUtils.nextAutoGeneratedName(userFacilities, 'LL');
            const icao = ICAO.value('U', '', '', ident);
            facility = UserFacilityUtils.createFromLatLon(icao, leg.lat, leg.lon);
            ...
            if (referenceFacility) {
              const ident = WT21PilotWaypointUtils.nextAutoGeneratedName(userFacilities, leg.referenceIcao.ident);
              const icao = ICAO.value('U', '', '', ident);
  ...
  private retrieveApproachIndexes(
    ...
    if (result.approachIndex >= 0 && transitionName !== '' && result.transitionIndex < 0) {
      console.warn(`WT21FlightPlanRouteLoader: unable to find approach transition with name '${transitionName}' in approach ${ICAO.getAirportIdentFromStringV1(airport.icao)}.${WT21FmsUtils.getApproachNameAsString(airport, result.approachIndex, -1)}`);
    }
