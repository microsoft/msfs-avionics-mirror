...
import './GtcAdvancedVnavCruiseTab.css';

/** The properties for the {@link GtcAdvancedVnavCruiseTab} component. */
interface GtcAdvancedVnavCruiseTabProps extends ComponentProps {
  /** The gtc service. */
...
/**
 * The Cruise tab of the advanced VNAV profile page.
 */
export class GtcAdvancedVnavCruiseTab extends DisplayComponent<GtcAdvancedVnavCruiseTabProps> implements GtcAdvancedVnavProfilePageTabContent {
  ...
  private readonly selectScheduleInputData = this.props.fmsSpeedSettingManager.cruiseSchedules.slice(1).map((schedule, index) => {
    return {
      value: index + 1,
  ...
  private readonly dialogChainSteps: [GtcDialogChainStep<GtcListDialog>, GtcDialogChainStep<GtcFmsSpeedDialog>, GtcDialogChainStep<GtcFmsSpeedDialog>] = [
    {
      ...
      onResult: (result: GtcDialogResult<any>) => {
        if (result.wasCancelled) {
          ...
          if ((result.payload as number) === this.props.fmsSpeedSettingManager.cruiseSchedules.length - 1) {
            ...
            if (index < 0 || index >= this.props.fmsSpeedSettingManager.cruiseSchedules.length) {
              index = 0;
            }
  ...
  private readonly schedule = this.props.fmsSpeedSettingManager.getSetting('fmsSpeedCruiseScheduleIndex').map(index => {
    const schedules = this.props.fmsSpeedSettingManager.cruiseSchedules;
  ...
  /**
   * Opens a dialog to allow the user to select a cruise schedule.
   */
  ...
  public render(): VNode {
    return (
      <div class="advanced-vnav-cruise-tab">
        <GtcAdvancedVnavScheduleButton
          gtcService={this.props.gtcService}
          label='Cruise Schedule'
          schedule={this.schedule}
          iasSetting={this.props.fmsSpeedSettingManager.getSetting('fmsSpeedCruiseIas')}
          machSetting={this.props.fmsSpeedSettingManager.getSetting('fmsSpeedCruiseMach')}
