...
export class PageLinkField extends DisplayField<string> {
  /** @inheritDoc */
  protected constructor(page: FmcPage, options: PageLinkFieldOptions) {
    const opts: DisplayFieldOptions<string> = {
      formatter: RawFormatter,
      style: options.disabled ? '[disabled]' : '',
      disabled: options.disabled,
      clearScratchpadOnSelectedHandled: false,
      onSelected: options.onSelected ?? (async (): Promise<boolean> => { page.setActiveRoute(options.route); return true; }),
    };
    super(page, opts);
   ...
   */
  static createLink(page: FmcPage, label: string, route: string, disabled = false): PageLinkField {
