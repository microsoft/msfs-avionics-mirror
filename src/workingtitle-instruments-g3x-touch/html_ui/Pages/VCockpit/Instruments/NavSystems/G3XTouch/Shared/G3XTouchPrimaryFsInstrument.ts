import {
  ...
  CdiControlEventsForId, ClockEvents, DefaultXmlAuralAlertParser, FSComponent, FlightPlanCalculatedEvent,
  FlightTimerInstrument, FlightTimerMode, GameStateProvider, GpsSynchronizer, LNavComputer, LNavObsManager,
  MappedSubject, MinimumsManager, MinimumsMode, NavComInstrument, NavSourceType, SetSubject, SimVarValueType,
  SoundServer, Subject, UnitType, UserSetting, VNavAltCaptureType, VNavPathMode, VNavState, Value, Wait,
...
import {
  ...
  GarminAutopilot, GarminObsLNavModule, GarminTimerControlEvents, GarminTimerManager, GarminVNavGlidepathGuidance,
  GarminVNavGuidance, GarminVNavPathGuidance, MinimumsUnitsManager, NavdataComputer, TrafficOperatingModeSetting
...
import { G3XFlightPlanSimSyncManager } from './FlightPlan/G3XFlightPlanSimSyncManager';
...
export class G3XTouchPrimaryFsInstrument extends G3XTouchFsInstrument {
  ...
  private readonly apConfig = this.config.autopilot
    ? new GarminAPConfig(this.bus, this.fplSourceDataProvider.fms, undefined, {
      cdiId: 'g3x',
      useIndicatedMach: true,
      lnavOptions: {
        steerCommand: this.gpsSteerCommand
      },
      useInternalVNavComputer: false,
  ...
  private readonly autoBacklightManager = new G3XAutoBacklightManager(this.bus);

  private globalSettingSaveManager?: G3XGlobalUserSettingSaveManager;

  private flightPlanSimSyncManager?: G3XFlightPlanSimSyncManager;
  ...
  protected async initInternalFlightPlans(): Promise<void> {
    ...
    this.flightPlanSimSyncManager = await G3XFlightPlanSimSyncManager.getManager(this.bus, this.fms);
