import {
  ActiveLegType, AdcEvents, AdditionalApproachType, AirportFacility, AirwayObject, AltitudeRestrictionType, ApproachProcedure, ApproachUtils,
  BitFlags, ConsumerSubject, ControlEvents, EventBus, ExtendedApproachType, Facility, FacilityLoader, FacilityRepository, FacilityType,
  FacilityUtils, FixTypeFlags, FlightPlan, FlightPlanCalculatedEvent, FlightPlanCopiedEvent, FlightPlanIndicationEvent, FlightPlanLeg,
  FlightPlanner, FlightPlannerEvents, FlightPlanOriginDestEvent, FlightPlanPredictor, FlightPlanSegment, FlightPlanSegmentType,
  GeoPoint, GeoPointInterface, GNSSEvents, ICAO, IntersectionFacility, LegDefinition, LegDefinitionFlags, LegTurnDirection,
  LegType, MagVar, NavEvents, NavMath, NavSourceId, NavSourceType, OneWayRunway, RnavTypeFlags, RunwayUtils, SmoothingPathCalculator,
  SpeedRestrictionType,
  SpeedUnit,
  Subject, UnitType, UserFacility, VerticalData, VerticalFlightPhase, VerticalFlightPlan, VisualFacility, VNavUtils, VorFacility,
} from '@microsoft/msfs-sdk';

import { CDIScaleLabel, WT21LNavDataEvents } from '../../FMC/Autopilot/WT21LNavDataEvents';
import { FmsPos } from '../../FMC/FmsPos';
import { FmcBtnEvents } from '../../FMC/Framework/FmcEventPublisher';
import {
  ApproachPerformanceManager
} from '../../FMC/PerformanceCalculators/AppraochPerformance/ApproachPerformanceManager';
import { BasePerformanceDataManager } from '../../FMC/PerformanceCalculators/BasePerformanceDataManager';
import {
  TakeoffPerformanceManager
} from '../../FMC/PerformanceCalculators/TakeoffPerformance/TakeoffPerformanceManager';
...
import { PerformancePlanProxy } from '../Performance/PerformancePlanProxy';
import { PerformancePlanRepository } from '../Performance/PerformancePlanRepository';
import { WT21ControlEvents } from '../WT21ControlEvents';
import { WT21FmsUtils, WT21LegDefinitionFlags } from './WT21FmsUtils';
...
export class WT21Fms {
  ...
  public readonly performancePlanProxy: PerformancePlanProxy = new PerformancePlanProxy(
    ...
    (property, newValue) => {
      if (property.editInPlace) {
        ...
        this.performancePlanRepository.triggerSync(WT21Fms.PRIMARY_ACT_PLAN_INDEX);
        ...
        if (modPerfPlan) {
          ...
          this.performancePlanRepository.triggerSync(WT21Fms.PRIMARY_MOD_PLAN_INDEX);
        }
  ...
  public readonly takeoffPerformanceManager = new TakeoffPerformanceManager(this.bus, this.basePerformanceManager, this.performancePlanProxy);
  public readonly approachPerformanceManager;
  ...
  public getActivePerformancePlan(): PerformancePlan {
    return this.performancePlanRepository.forFlightPlanIndex(WT21Fms.PRIMARY_ACT_PLAN_INDEX);
  }
  ...
  constructor(
    ...
    const nominalActiveLegSubject = ConsumerSubject.create(this.bus.getSubscriber<WT21LNavDataEvents>().on('lnavdata_nominal_leg_index'), -1);

    // This is updated in the ApproachPerformanceManager
    const approachPerformanceManagerPredictor = new FlightPlanPredictor(
      this.bus,
      this.flightPlanner,
      Subject.create(WT21Fms.PRIMARY_ACT_PLAN_INDEX),
      nominalActiveLegSubject,
      WT21FlightPlanPredictorConfiguration,
    );

    this.approachPerformanceManager = new ApproachPerformanceManager(this.bus, this, this.basePerformanceManager, this.performancePlanProxy, approachPerformanceManagerPredictor);
  ...
  public async initPrimaryFlightPlan(): Promise<void> {
    if (this.flightPlanner.hasFlightPlan(WT21Fms.PRIMARY_ACT_PLAN_INDEX)) {
      this.setFacilityInfo();
      return;
    }

    this.flightPlanner.createFlightPlan(WT21Fms.PRIMARY_ACT_PLAN_INDEX);
    this.flightPlanner.createFlightPlan(WT21Fms.PRIMARY_MOD_PLAN_INDEX);
   ...
   */
  public getFlightPlan(index = WT21Fms.PRIMARY_ACT_PLAN_INDEX): FlightPlan {
  ...
  public hasPrimaryFlightPlan(): boolean {
    return this.flightPlanner.hasFlightPlan(WT21Fms.PRIMARY_ACT_PLAN_INDEX);
  }
  ...
  public getPrimaryFlightPlan(): FlightPlan {
    return this.flightPlanner.getFlightPlan(WT21Fms.PRIMARY_ACT_PLAN_INDEX);
  }
  ...
  private getPrimaryVerticalFlightPlan(): VerticalFlightPlan {
    return this.verticalPathCalculator.getVerticalFlightPlan(WT21Fms.PRIMARY_ACT_PLAN_INDEX);
  }
  ...
  public hasPrimaryModFlightPlan(): boolean {
    return this.flightPlanner.hasFlightPlan(WT21Fms.PRIMARY_MOD_PLAN_INDEX);
  }
  ...
  private getPrimaryModFlightPlan(): FlightPlan {
    return this.flightPlanner.getFlightPlan(WT21Fms.PRIMARY_MOD_PLAN_INDEX);
  }
  ...
  private getPrimaryModVerticalFlightPlan(): VerticalFlightPlan {
    return this.verticalPathCalculator.getVerticalFlightPlan(WT21Fms.PRIMARY_MOD_PLAN_INDEX);
  }

  /**
   * Handles when the EXEC button is pressed and copies the MOD plan back to the PRIMARY plan
   */
  private onExecButtonPressed = (): void => {
    if (this.planInMod.get()) {
      const modPlan = this.getModFlightPlan();

      if (this.getDirectToState(WT21Fms.PRIMARY_MOD_PLAN_INDEX) === DirectToState.TOEXISTING) {
      ...
      this.flightPlanner.copyFlightPlan(WT21Fms.PRIMARY_MOD_PLAN_INDEX, WT21Fms.PRIMARY_ACT_PLAN_INDEX, true);
  ...
  public getModFlightPlan(): FlightPlan {
    if (!this.planInMod.get()) {
      this.flightPlanner.copyFlightPlan(WT21Fms.PRIMARY_ACT_PLAN_INDEX, WT21Fms.PRIMARY_MOD_PLAN_INDEX, true);
  ...
  public getPlanIndexForFmcPage(): number {
    if (this.planInMod.get()) {
      return WT21Fms.PRIMARY_MOD_PLAN_INDEX;
    }
    return WT21Fms.PRIMARY_ACT_PLAN_INDEX;
  }
  ...
  private onPlanCalculated = (e: FlightPlanCalculatedEvent): void => {
    if (e.planIndex === WT21Fms.PRIMARY_ACT_PLAN_INDEX && this.planInMod.get()) {
      ...
      if (this.getDirectToState(WT21Fms.PRIMARY_MOD_PLAN_INDEX)) {
  ...
  public getFlightPlanAlternate(planIndex?: number): string | undefined {
    ...
    return plan.getUserData(WT21Fms.USER_DATA_KEY_ALTN);
  }
  ...
  public setFlightPlanAlternate(facility: AirportFacility | undefined): void {
    ...
    plan.setUserData(WT21Fms.USER_DATA_KEY_ALTN, facility?.icao ?? undefined);
  }
   ...
   */
  private removeDirectToExisting(planIndex = WT21Fms.PRIMARY_MOD_PLAN_INDEX, lateralLegIndex?: number, calculate = true): void {
    const plan = this.getFlightPlan(planIndex);
   ...
   */
  public getDirectToState(planIndex = WT21Fms.PRIMARY_ACT_PLAN_INDEX): DirectToState {
  ...
  private buildDepartureLegs(facility: AirportFacility,
    ...
    let originLeg;
    if (oneWayRunway) {
      originLeg = WT21FmsUtils.buildRunwayLeg(facility, oneWayRunway, true);
    } else {
  ...
  public insertArrival(
    ...
    const directToState = this.getDirectToState(WT21Fms.PRIMARY_MOD_PLAN_INDEX);
  ...
  private moveDirectToDestinationLeg(plan: FlightPlan, targetSegmentType: FlightPlanSegmentType, arrivalSegmentIndex?: number): boolean {
    ...
    if (directTargetSegmentIndex !== undefined && directTargetSegmentIndex > 0 && plan.getLeg(plan.activeLateralLeg).leg.fixIcao === plan.destinationAirport) {
      ...
      plan.setCalculatingLeg(newActiveLegIndex);
      plan.setLateralLeg(newActiveLegIndex);
      plan.planIndex !== WT21Fms.PRIMARY_ACT_PLAN_INDEX && plan.calculate(newActiveLegIndex);
      return true;
    }
  ...
  public createDirectTo(segmentIndex?: number, segmentLegIndex?: number, isNewDTO = true, course?: number, facility?: Facility): void {
    ...
    if (plan.directToData.segmentIndex > -1 && plan.directToData.segmentLegIndex > -1) {
      ...
      if (this.getDirectToState(WT21Fms.PRIMARY_MOD_PLAN_INDEX) === DirectToState.TOEXISTING) {
        this.removeDirectToExisting(WT21Fms.PRIMARY_MOD_PLAN_INDEX, undefined, false);
      } else {
  ...
  public createVerticalDirectTo(
    ...
    const distanceToConstraint = isAutomatic
      ...
      : WT21FmsUtils.getDistanceFromPposToLegEnd(plan, globalIndex);
    ...
    this.verticalPathCalculator.requestPathCompute(WT21Fms.PRIMARY_MOD_PLAN_INDEX);
  ...
  private tryUpdatePposHoldPosition(plan: FlightPlan): void {
    ...
    if (plan.planIndex !== WT21Fms.PRIMARY_MOD_PLAN_INDEX || pposHoldInActive || !modActiveLeg || notPposHold) {
  ...
  public async emptyPrimaryFlightPlan(): Promise<void> {
    ...
    const plan = this.flightPlanner.getFlightPlan(WT21Fms.PRIMARY_ACT_PLAN_INDEX);
  ...
  private procPreviewOpId = 0;

  /**
   * Builds a temporary flight plan to preview a procedure.
   ...
   * @returns A Promise which is fulfilled with the index of the procedure preview flight plan when the procedure
   * preview is built.
   */
  public async buildProcedurePreviewPlan(
    ...
    this.flightPlanner.deleteFlightPlan(WT21Fms.PROC_PREVIEW_PLAN_INDEX);
    const plan = this.flightPlanner.createFlightPlan(WT21Fms.PROC_PREVIEW_PLAN_INDEX);

    const opId = ++this.procPreviewOpId;
    ...
    if (procedureLegObject && procedureLegObject.procedureLegs.length > 0) {
      ...
      procedureLegObject.procedureLegs.forEach((l) => {
        plan.addLeg(0, l, undefined, l.WT21LegDefinitionFlags ?? WT21LegDefinitionFlags.None, false);
      });
        ...
        if (!visualRunwayNumber && !visualRunwayDesignator && procedureLegObject.procedureLegs.length > 0) {
          ...
          if (missedLegs && missedLegs.length > 0) {
            ...
            for (let n = 0; n < missedLegs.length; n++) {
              ...
              if (maphIndex > 0 && n === maphIndex) {
                newLeg.fixTypeFlags |= FixTypeFlags.MAHP;
                plan.addLeg(0, newLeg, undefined, WT21LegDefinitionFlags.MissedApproach, false);
              } else {
                plan.addLeg(0, newLeg, undefined, WT21LegDefinitionFlags.MissedApproach, false);
              }
            }
  ...
  public buildAirwayPreviewSegment(airway: AirwayObject, entry: IntersectionFacility, exit: IntersectionFacility): number {
    ...
    if (airwayLegObject.procedureLegs.length > 0) {
      airwayLegObject.procedureLegs.forEach((l) => {
        plan.addLeg(0, l, undefined, WT21LegDefinitionFlags.None, false);
      });
      plan.calculate(0, true);
    }
    return WT21Fms.PROC_PREVIEW_PLAN_INDEX;
  }
