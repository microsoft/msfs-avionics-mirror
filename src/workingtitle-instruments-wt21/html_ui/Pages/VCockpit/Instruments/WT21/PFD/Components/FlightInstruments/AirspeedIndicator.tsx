import {
  AdcEvents, AvionicsSystemState, AvionicsSystemStateEvent, ConsumerValue, DisplayComponent, EventBus, ExpSmoother, FSComponent, MathUtils, NodeReference,
  SimVarValueType, Subject, VNode
...
import { RefSpeedType, VSpeedType } from '../../../Shared/ReferenceSpeeds';
import { AOASystemEvents } from '../../../Shared/Systems';
...
export class AirspeedIndicator extends DisplayComponent<AirspeedIndicatorProps> {
  ...
  private buildScroller(startYValue = -2, blankZeroValue = false): SVGTextElement[] {
    ...
    for (let i = 11; i > -3; i--) {
      ...
      scroller.push(<text x='8' y={yValue} fill="var(--wt21-colors-green)" text-anchor="middle" font-size='32'>{numberText}</text>);
      yValue += 30;
    }

    scroller.push(<text x='8' y={yValue + 30} fill="var(--wt21-colors-green)" text-anchor="middle" font-size='32'>-</text>);
    return scroller;
  }
  ...
  private onAdcStateChanged(state: AvionicsSystemStateEvent): void {
    this.airspeedContainerRef.instance.classList.toggle('fail', (state.current == AvionicsSystemState.Failed || state.current == AvionicsSystemState.Initializing));
  }
  ...
  public render(): VNode {
    return (
      <div>
        ...
        <div class="translucent-box airspeed-box" ref={this.airspeedContainerRef}>
          <div class="airspeed">
