import { WT21ChecklistDomParser } from '../Checklist/WT21ChecklistDomParser';
import { WT21ChecklistSetDef } from '../Checklist/WT21ChecklistDefinition';

/**
 * A configuration object which defines checklist options.
 */
// TODO(Vito): Perhaps extract this into the SDK
export class ChecklistConfig {
  ...
  /** @inheritDoc */
  public resolve(): (timeout: number) => Promise<WT21ChecklistSetDef | undefined> {
            ...
            if (request.readyState === XMLHttpRequest.DONE) {
              if (request.status === 200) {
                try {
                  ...
                  if (rootElement) {
                    const def = new WT21ChecklistDomParser().parse(rootElement, {
