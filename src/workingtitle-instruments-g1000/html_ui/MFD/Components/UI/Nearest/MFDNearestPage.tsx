import {
  AhrsEvents,
  CompiledMapSystem, ConsumerSubject, ControlPublisher, EventBus, Facility, FacilityLoader, FocusPosition, FSComponent, GeoPoint, GNSSEvents,
  MapIndexedRangeModule, MapSystemBuilder, MathUtils, NearestSubscription, Subscription, Vec2Math, VecNMath, VNode
...
export abstract class MFDNearestPage<T extends Facility, P extends MFDNearestPageProps = MFDNearestPageProps> extends MFDUiPage<P> {
  ...
  private readonly planeHeading = ConsumerSubject.create(this.props.bus.getSubscriber<AhrsEvents>().on('hdg_deg_true').atFrequency(1), 0).pause();

  private readonly positionSub: Subscription;
  ...
  constructor(props: P) {
    ...
    this.positionSub = this.props.bus.getSubscriber<GNSSEvents>().on('gps-position').atFrequency(1).handle(this.onGps, true);
  }
