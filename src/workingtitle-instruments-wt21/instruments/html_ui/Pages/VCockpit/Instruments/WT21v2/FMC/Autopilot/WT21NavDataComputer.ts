import {
  ...
  LegDefinition, LegDefinitionFlags, LegType, LNavEvents, LNavTransitionMode, LNavUtils, MagVar, MathUtils, NavEvents, NavSourceId, NavSourceType,
  Publisher, RnavTypeFlags, Subject, UnitType
} from '@microsoft/msfs-sdk';

import { ApproachDetails, CDIScaleLabel, FMS_MESSAGE_ID, MessageService, WT21ControlEvents, WT21FmsUtils, WT21LNavDataEvents } from '@microsoft/msfs-wt21-shared';
 ...
 */
class LNavDataTopicEntry<T extends keyof WT21LNavDataEvents> {
  /** The topic name to which this entry publishes. */
  public topic: T;

  /** The value of this entry's topic data. */
  public value: WT21LNavDataEvents[T];
   ...
   */
  public constructor(private readonly publisher: Publisher<WT21LNavDataEvents>, topic: T, initialValue: WT21LNavDataEvents[T]) {
    this.topic = topic;
   ...
   */
  public publish(value?: WT21LNavDataEvents[T]): void {
    if (value !== this.value) {
      if (value !== undefined) {
        (this.value as WT21LNavDataEvents[T]) = value;
      }
...
export class WT21NavDataComputer {
  ...
  private readonly eventBusTopicEntries: {
    [P in keyof WT21LNavDataEvents]: LNavDataTopicEntry<P>;
  };
  ...
  constructor(
    ...
    const publisher = this.bus.getPublisher<WT21LNavDataEvents>();
    ...
    this.eventBusTopicEntries = {
      ...
      'lnavdata_cdi_scale_label': new LNavDataTopicEntry(publisher, 'lnavdata_cdi_scale_label', CDIScaleLabel.Enroute),
  ...
  private republishEventBusTopics(): void {
    for (const topic in this.eventBusTopicEntries) {
      this.eventBusTopicEntries[topic as keyof WT21LNavDataEvents].publish();
    }
  }
  ...
  private updateCDIScaling(): void {
    ...
    const currentScaling = this.eventBusTopicEntries['lnavdata_cdi_scale_label'].value;
    const isInNorthAtlantic = this.planePos.lat >= 27 && this.planePos.lat <= 67 &&
      this.planePos.lon >= -60 && this.planePos.lon <= -10;

    this.eventBusTopicEntries['lnavdata_destination_distance_direct'].publish(distanceToArivalNM);
    ...
    if (currentScaling === CDIScaleLabel.Approach) {
      if (isWithin2NmOfFAF) {
        ...
        this.eventBusTopicEntries['lnavdata_cdi_scale'].publish(newScaling);
      } else {
        this.eventBusTopicEntries['lnavdata_cdi_scale'].publish(1);
      }
  ...
  private setEnRouteScaling(): void {
    this.eventBusTopicEntries['lnavdata_cdi_scale'].publish(2);
    this.eventBusTopicEntries['lnavdata_cdi_scale_label'].publish(CDIScaleLabel.Enroute);
  }

  // eslint-disable-next-line jsdoc/require-jsdoc
  private setOceanicScaling(): void {
    this.eventBusTopicEntries['lnavdata_cdi_scale'].publish(4);
    this.eventBusTopicEntries['lnavdata_cdi_scale_label'].publish(CDIScaleLabel.Oceanic);
  }

  // eslint-disable-next-line jsdoc/require-jsdoc
  private setTerminalDepartureScaling(): void {
    this.eventBusTopicEntries['lnavdata_cdi_scale'].publish(1);
    this.eventBusTopicEntries['lnavdata_cdi_scale_label'].publish(CDIScaleLabel.TerminalDeparture);
  }

  // eslint-disable-next-line jsdoc/require-jsdoc
  private setTerminalArrivalScaling(): void {
    this.eventBusTopicEntries['lnavdata_cdi_scale'].publish(1);
    this.eventBusTopicEntries['lnavdata_cdi_scale_label'].publish(CDIScaleLabel.TerminalArrival);
  }
  ...
  private setApproachScaling(): void {
    ...
    this.eventBusTopicEntries['lnavdata_cdi_scale'].publish(1);
    this.eventBusTopicEntries['lnavdata_cdi_scale_label'].publish(CDIScaleLabel.Approach);
  }

  // eslint-disable-next-line jsdoc/require-jsdoc
  private setMissedApproachScaling(): void {
    this.eventBusTopicEntries['lnavdata_cdi_scale'].publish(0.3);
    this.eventBusTopicEntries['lnavdata_cdi_scale_label'].publish(CDIScaleLabel.MissedApproach);
  }
  ...
  private isOceanicScaling(): boolean {
    return this.eventBusTopicEntries['lnavdata_cdi_scale_label'].value === CDIScaleLabel.Oceanic;
  }

  // eslint-disable-next-line jsdoc/require-jsdoc
  private isTerminalScaling(): boolean {
    return terminalScalingLabels.includes(this.eventBusTopicEntries['lnavdata_cdi_scale_label'].value);
  }

  // eslint-disable-next-line jsdoc/require-jsdoc
  private isTerminalArrivalScaling(): boolean {
    return this.eventBusTopicEntries['lnavdata_cdi_scale_label'].value === CDIScaleLabel.TerminalArrival;
  }

  // eslint-disable-next-line jsdoc/require-jsdoc
  private isApproachScaling(): boolean {
    return this.eventBusTopicEntries['lnavdata_cdi_scale_label'].value === CDIScaleLabel.Approach
      || this.eventBusTopicEntries['lnavdata_cdi_scale_label'].value === CDIScaleLabel.MissedApproach;
  }
  ...
  private isWithin2NmOfFAF(flightPlan: FlightPlan, currentLeg: LegDefinition, fafIndex?: number): [boolean, number] {
    if (currentLeg?.calculated?.endLat === undefined || currentLeg?.calculated?.endLon === undefined || fafIndex === undefined) {
      this.eventBusTopicEntries['lnavdata_distance_to_faf'].publish(Number.MAX_SAFE_INTEGER);
    ...
    this.eventBusTopicEntries['lnavdata_distance_to_faf'].publish(sumNM);
