import {
  AdcEvents, Animator, APEvents, AvionicsSystemState, AvionicsSystemStateEvent, ComponentProps, DefaultUserSettingManager,
  DisplayComponent, Easing, EventBus, FSComponent, MathUtils, MinimumsEvents, MinimumsMode,
...
import { ADCSystemEvents } from '../../../Shared/Systems/ADCSystem';
import { RASystemEvents } from '../../../Shared/Systems/RASystem';
  ...
  private buildDoubleScroller(startYValue = 78): SVGTextElement[] {
    ...
    for (let i = 0; i < 15; i++) {
      ...
      if (i == 5 || i == 7) {
        ...
        scroller.push(<text x='15' y={yValue} class={altClassName} fill="var(--wt21-colors-green)" text-anchor="middle" font-size='22'>{altNumberText}</text>);
        className += i == 5 ? ' top show-above-msl' : ' bottom show-below-msl';
      }
      scroller.push(<text x='15' y={yValue} class={className} fill="var(--wt21-colors-green)" text-anchor="middle" font-size='22'>{numberText}</text>);
  ...
  private buildSingleScroller(startYValue = 0, includeNeg = false): SVGTextElement[] {
    ...
    for (let i = 0; i < 24; i++) {
      ...
      if (i == 10 || i == 12) {
        ...
        scroller.push(<text x='8' y={yValue} class={altClassName} fill="var(--wt21-colors-green)" text-anchor="middle" font-size='34'>{altNumberText}</text>);
      ...
      scroller.push(<text x='8' y={yValue} class={className} fill="var(--wt21-colors-green)" text-anchor="middle" font-size='34'>{numberText}</text>);

      yValue += 47;
    }

    if (includeNeg) {
      scroller.push(<text x='8' y={yValue + 48.5} fill="var(--wt21-colors-green)" text-anchor="middle" font-size='34'>$</text>);
      scroller.push(<text x='5' y={yValue + 60 + 47 - 20} fill="var(--wt21-colors-green)" text-anchor="middle" font-size='22'>N</text>);
      scroller.push(<text x='5' y={yValue + 60 + 47} fill="var(--wt21-colors-green)" text-anchor="middle" font-size='22'>E</text>);
      scroller.push(<text x='5' y={yValue + 60 + 47 + 20} fill="var(--wt21-colors-green)" text-anchor="middle" font-size='22'>G</text>);
    }
  ...
  public render(): VNode {
    return (
      <div ref={this.altimeterContainerRef}>
        <AltPreselectBox bus={this.props.bus} />
            ...
            <div class="altitude-ground-group" ref={this.altitudeTapeGroundElement} style="transform:translate(0,300px)">
              ...
              <div class="altitude-ground-ribbon">
                <svg width="140" height="415" viewBox="0 0 140 415">
                  ...
                  <rect fill="var(--wt21-colors-amber)" stroke="var(--wt21-colors-yellow)" stroke-width="2" width="140" x="0"
                    height="2"></rect>
                </svg>
              </div>
            </div>
