import { Epic2FlightPlans, Epic2Fms, Epic2FmsUtils } from '@microsoft/msfs-epic2-shared';
import {
  AirportFacility, AirportUtils, AirwayData, AirwayType, ApproachIdentifier, ArrayUtils, Facility,
  FacilityType,
  FacilityUtils, FlightPlan, FlightPlanSegmentType, FlightPlanUtils, GeoPoint, ICAO, IcaoValue, IntersectionFacility,
  LegDefinition, LegType, MagVar, OneWayRunway, Procedure, ReadonlyFlightPlanRoute, RunwayIdentifier, RunwayUtils,
  UserFacility
} from '@microsoft/msfs-sdk';
import { Epic2FlightPlanRouteUtils } from './Epic2FlightPlanRouteUtils';

/**
 * A loader of flight plan routes into an instance of {@link Epic2Fms}.
 */
export class Epic2FlightPlanRouteLoader {
  private loadOpId = 0;
  private activeLoadPromise?: Promise<boolean>;

  /**
   * Creates a new instance of Epic2FlightPlanRouteLoader.
   * @param fms The FMS containing the primary flight plan to which the loader loads flight plan routes.
   */
  public constructor(private readonly fms: Epic2Fms) {
  }
   ...
   * @param exec Whether to execute the changes to the flight plan after the route is loaded.
  ...
  public loadRoute(route: ReadonlyFlightPlanRoute, exec: boolean): Promise<boolean> {
    return this.scheduleLoadFromRoute(route, exec);
  }
   ...
   * rejected.
   */
  private async scheduleLoadFromRoute(route: ReadonlyFlightPlanRoute, exec: boolean): Promise<boolean> {
    ...
    const promise = this.loadFromRoute(opId, route, exec);
   ...
   */
  private async loadFromRoute(opId: number, route: ReadonlyFlightPlanRoute, exec: boolean): Promise<boolean> {
    ...
    try {
      ...
      plan = this.fms.getModFlightPlan();
      ...
      batchUuid = plan.openBatch(Epic2FlightPlanRouteUtils.DEFAULT_LOAD_ROUTE_BATCH_NAME);

      this.fms.emptyFlightPlan(Epic2FlightPlans.Pending);
      ...
      if (ICAO.isValueFacility(route.destinationAirport, FacilityType.Airport)) {
        ...
        if (destination) {
          ...
          if (approachIndexes.approachIndex >= 0) {
            // { facility: destination, approachIndex, approachTransitionIndex }
      ...
      if (Epic2FmsUtils.canDirectTo(plan, directToTargetIndexes.segmentIndex, directToTargetIndexes.segmentLegIndex)) {
  ...
  private async loadEnrouteLegs(opId: number, plan: FlightPlan, route: ReadonlyFlightPlanRoute): Promise<LegDefinition | null> {
    ...
    const tmpGeoPoint = new GeoPoint(0, 0);
    ...
    for (const leg of route.enroute) {
      ...
      if (leg.hasLatLon || leg.hasPointBearingDistance) {
        ...
        try {
          ...
          if (leg.hasLatLon) {
            tmpGeoPoint.set(leg.lat, leg.lon);
            facility = this.fms.createLLWaypoint(tmpGeoPoint);
            ...
            if (referenceFacility) {
              facility = this.fms.createPBDWaypoint(referenceFacility, MagVar.trueToMagnetic(leg.bearing, FacilityUtils.getMagVar(referenceFacility)), leg.distance);
            }
          }
          ...
          console.warn(`Epic2FlightPlanRouteLoader: unable to create user facility from leg ${leg}`);
        }
      } else {
        if (ICAO.isValueFacility(leg.fixIcao)) {
            ...
            if (leg.via !== '') {
              ...
              if (airwayEntryFacility) {
                ...
                if (airwayExitFacility) {
                  ...
                  console.warn(`Epic2FlightPlanRouteLoader: could not load airway '${leg.via}' to fix ${leg.fixIcao.ident} - the fix is not a valid exit waypoint`);
                }
              } else {
                console.warn(`Epic2FlightPlanRouteLoader: could not load airway '${leg.via}' to fix ${leg.fixIcao.ident} - no valid entry waypoint`);
              }
            }
          ...
          console.warn(`Epic2lightPlanRouteLoader: enroute fix with ICAO '${ICAO.valueToStringV1(leg.fixIcao)}' has invalid facility type`);
        }
      }
  ...
  private async retrieveFacility(icao: IcaoValue): Promise<Facility | null> {
    try {
      ...
      console.warn(`Epic2FlightPlanRouteLoader: unable to retrieve facility with ICAO '${ICAO.valueToStringV1(icao)}'`);
    }
    ...
    if (ICAO.isValueFacility(airportIcao, FacilityType.Airport)) {
      ...
      if (airport) {
        ...
        if (runwayIdentifier.number !== '') {
          ...
          if (!runway) {
            console.warn(`Epic2FlightPlanRouteLoader: unable to retrieve find runway with designation '${designation}' in airport ${airportIcao.ident}`);
          }
        }
  ...
  private retrieveDepartureArrivalIndexes(
    ...
    if (procedureName !== '' && result.procedureIndex < 0) {
      console.warn(`Epic2FlightPlanRouteLoader: unable to find procedure with name '${procedureName}' in airport ${ICAO.getAirportIdentFromStringV1(airport.icao)}`);
    }

    if (result.procedureIndex >= 0 && transitionName !== '' && result.enrouteTransitionIndex < 0) {
      console.warn(`Epic2FlightPlanRouteLoader: unable to find enroute transition with name '${transitionName}' in procedure ${ICAO.getAirportIdentFromStringV1(airport.icao)}.${procArray[result.procedureIndex].name}`);
    }
  ...
  private retrieveApproachIndexes(
    ...
    if (approachIdentifier.type !== '' && result.approachIndex < 0) {
      console.warn(`Epic2FlightPlanRouteLoader: unable to find approach ${approachIdentifier.type}${approachIdentifier.suffix === '' ? '' : ` ${approachIdentifier.suffix}`}${approachIdentifier.runway.number === '' ? '' : ` for runway ${approachIdentifier.runway.number}${approachIdentifier.runway.designator}`} in airport ${ICAO.getAirportIdentFromStringV1(airport.icao)}`);
    }

    if (result.approachIndex >= 0 && transitionName !== '' && result.transitionIndex < 0) {
      console.warn(`Epic2FlightPlanRouteLoader: unable to find approach transition with name '${transitionName}' in approach ${ICAO.getAirportIdentFromStringV1(airport.icao)}.${Epic2FmsUtils.getApproachNameAsString(airport, result.approachIndex, -1)}`);
    }
      ...
      switch (ICAO.getFacilityTypeFromValue(icao)) {
        ...
        case FacilityType.Intersection:
          try {
            ...
            console.warn(`Epic2FlightPlanRouteLoader: unable to retrieve waypoint with ICAO '${ICAO.valueToStringV1(icao)}'`);
          }
      }
    }
  ...
  > {
    if (ICAO.valueEquals(entryFacility.icaoStruct, exitFacility.icaoStruct)) {
      console.warn(`Epic2FlightPlanRouteLoader: could not load airway '${airwayName}' to fix ${ICAO.tryValueToStringV2(exitFacility.icaoStruct)} - the fix is the same as the entry fix`);
      return null;
    }
    ...
    try {
      ...
      if ((exitIndex = airway.waypoints.findIndex(airwayFix => ICAO.valueEquals(airwayFix.icaoStruct, exitFacility.icaoStruct))) >= 0) {
        ...
        if ((entryIndex = airway.waypoints.findIndex(airwayFix => ICAO.valueEquals(airwayFix.icaoStruct, entryFacility.icaoStruct))) >= 0) {
          ...
          console.warn(`Epic2FlightPlanRouteLoader: could not load airway '${airwayName}' to fix ${ICAO.tryValueToStringV2(exitFacility.icaoStruct)} - the entry fix ${ICAO.tryValueToStringV2(entryFacility.icaoStruct)} is not part of the specified airway`);
        }
      } else {
        // Should never happen.
        console.warn(`Epic2FlightPlanRouteLoader: could not load airway '${airwayName}' to fix ${ICAO.tryValueToStringV2(exitFacility.icaoStruct)} - the fix is not part of the specified airway`);
      }
    } catch {
      console.warn(`Epic2FlightPlanRouteLoader: could not load airway '${airwayName}' to fix ${ICAO.tryValueToStringV2(exitFacility.icaoStruct)} - the fix is not part of the specified airway`);
    }
