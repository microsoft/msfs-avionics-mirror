import { Epic2ChecklistDef } from '../Checklists';
import { Epic2ChecklistDOMParser } from '../Checklists/Epic2ChecklistDOMParser';
...
export class ChecklistConfig {
  ...
  /** @inheritDoc */
  public resolve(): (timeout: number) => Promise<Epic2ChecklistDef | undefined> {
    if (this.checklistFileURL === undefined) {
      return () => Promise.resolve(undefined);
    } else {
      return (timeout: number) => {
        return new Promise<Epic2ChecklistDef | undefined>(resolve => {
            ...
            if (request.readyState === XMLHttpRequest.DONE) {
              if (request.status === 200) {
                try {
                  ...
                  if (rootElement) {
                    const def = new Epic2ChecklistDOMParser().parse(rootElement, {
