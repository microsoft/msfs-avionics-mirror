...
import {
  MapUserSettings, MapWaypointsDisplay, MfdDisplayMode, MFDUpperWindowState, MFDUserSettings, WT21InstrumentType, WT21MfdTextPage
...
export class DisplayMenu extends WT21FmcPage {
  ...
  private currentSourceInstrument: WT21InstrumentType.Pfd | WT21InstrumentType.Mfd = WT21InstrumentType.Mfd;
  private currentSourceIndex: 1 | 2 = this.screen.fmcIndex;
  ...
  private pfd1SettingsHandler = (): any => this.currentSourceInstrument === WT21InstrumentType.Pfd && this.currentSourceIndex === 1 && this.notifySettingsChanged();
  private pfd2SettingsHandler = (): any => this.currentSourceInstrument === WT21InstrumentType.Pfd && this.currentSourceIndex === 2 && this.notifySettingsChanged();
  private mfd1SettingsHandler = (): any => this.currentSourceInstrument === WT21InstrumentType.Mfd && this.currentSourceIndex === 1 && this.notifySettingsChanged();
  private mfd2SettingsHandler = (): any => this.currentSourceInstrument === WT21InstrumentType.Mfd && this.currentSourceIndex === 2 && this.notifySettingsChanged();

  private displayChangeHandler = (v: number): void => {
    this.currentSourceInstrument = v === 0 ? WT21InstrumentType.Mfd : WT21InstrumentType.Pfd;
    this.currentSourceIndex = this.currentSide.get() + 1 as 1 | 2;

    this.notifySettingsChanged();
  };

  private sideChangeHandler = (v: number): void => {
    this.currentSourceIndex = v + 1 as 1 | 2;

    this.notifySettingsChanged();
  };

  private rngAltSelHandler = (v: boolean): void => {
    this.mapSettings.getSetting(`mapAltitudeArcShow${this.currentSourceInstrument}_${this.currentSourceIndex}`).set(v);
  };

  private fmsTextChangeHandler = (option: number): void => {
    if (this.currentSourceInstrument !== WT21InstrumentType.Mfd) {
      return;
    }

    const dataWindowSetting = this.mfdSettings.getSetting(`mfdUpperWindowState_${this.currentSourceIndex}`);
    const vnavSetting = this.mfdSettings.getSetting(`mfdUpperFmsTextVNavShow_${this.currentSourceIndex}`);
  ...
  protected onInit(): void {
    ...
    this.addBinding(this.mapSettings.whenSettingChanged('mapWaypointsDisplayPFD_1').handle(this.pfd1SettingsHandler));
    this.addBinding(this.mapSettings.whenSettingChanged('mapWaypointsDisplayPFD_2').handle(this.pfd2SettingsHandler));
    ...
    this.addBinding(this.mapSettings.whenSettingChanged('mapAltitudeArcShowPFD_1').handle(this.pfd1SettingsHandler));
    this.addBinding(this.mapSettings.whenSettingChanged('mapAltitudeArcShowPFD_2').handle(this.pfd2SettingsHandler));
  ...
  private notifySettingsChanged(): void {
    this.currentWaypointDisplaySettings.set(this.mapSettings.getSetting(`mapWaypointsDisplay${this.currentSourceInstrument}_${this.currentSourceIndex}`).value);
    this.rngAltSelOption.set(this.mapSettings.getSetting(`mapAltitudeArcShow${this.currentSourceInstrument}_${this.currentSourceIndex}`).value);
  ...
  private createToggleField(flag: number, text: string): DisplayField<number> {
    return new DisplayField<number>(this, {
      ...
      onSelected: async (): Promise<boolean> => {
        const setting = this.mapSettings.getSetting(`mapWaypointsDisplay${this.currentSourceInstrument}_${this.currentSourceIndex}`);
    ...
    if (mfdMode === MfdDisplayMode.Map) {
      return [
        [
          ...
          [this.currentSourceInstrument === WT21InstrumentType.Pfd ? '' : ' WINDOW[blue]', 'SIDE [blue]'],
          [this.currentSourceInstrument === WT21InstrumentType.Pfd ? '' : this.fmsTextSwitch, this.SideSwitch],
        ],
        [
          [title, '2/2 [blue]', ''],
