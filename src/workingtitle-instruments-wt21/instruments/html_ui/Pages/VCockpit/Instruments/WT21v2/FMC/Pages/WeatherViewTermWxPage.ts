import { DisplayField, FmcPageLifecyclePolicy, FmcRenderTemplate, ICAO, PageLinkField, Subject } from '@microsoft/msfs-sdk';
 ...
 */
export class WeatherViewTermWxPage extends WT21FmcPage {
    ...
    const field = new DisplayField<string | null>(this, {
      ...
      onSelected: async (): Promise<boolean | string> => {
        if (this.wxSystem.getRequests()[index].airport !== undefined) {
          this.screen.navigateTo('/dl-terminalwx-single-view', { index: index });
        }
        return true;
      },
    });
