...
import './G3XPitchLadder.css';

/**
 * Styling options for the G3X Touch pitch ladder.
 */
export type G3XPitchLadderStyles = {
 ...
 */
export type G3XPitchLadderOptions = {
 ...
 */
export interface G3XPitchLadderProps extends HorizonLayerProps {
 ...
 */
export class G3XPitchLadder extends HorizonLayer<G3XPitchLadderProps> {
  ...
  private rebuildLadder(): void {
    ...
    const majorLineHeight = styles.majorLineHeight;
    ...
    for (let i = 1; i <= len; i++) {
      ...
      if (lineLength !== undefined) {
        if (lineLength > 0) {
          ...
          if (i % minorFactor === 0) {
            // major line
            FSComponent.render(<line x1={-lineLength / 2} y1={-y} x2={-lineLength / 2} y2={-y + majorLineHeight}
              class='pitch-ladder-line'>.</line>, this.svgRef.instance);
            FSComponent.render(<line x1={lineLength / 2} y1={-y} x2={lineLength / 2} y2={-y + majorLineHeight}
              class='pitch-ladder-line'>.</line>, this.svgRef.instance);
            ...
            if (this.props.includeUnusualAttitudeChevrons) {
              const lastMajorPitch = pitch - styles.majorLineIncrement;
              const widthFactor = 0.5 + (pitch / 90) * 0.4;
              const height = 0.95 * majorLineSeparation;
