import {
  AdcEvents, ConsumerSubject, ElectricalEvents, EventBus, KeyEventManager, MappedSubject, SimVarValueType,
  Subscription, Wait
...
export class WeatherRadarManager {
  ...
  constructor(
    ...
    if (scanActiveCircuitIndex !== undefined) {
      if (scanActiveProcedureIndex !== undefined) {
        KeyEventManager.getManager(bus).then(manager => {
          this.keyEventManager = manager;
          this.isScanActiveCircuitLogicReady = true;
          if (this.isInit) {
      ...
      this.scanActiveCircuitSimVar = `CIRCUIT SWITCH ON:${scanActiveCircuitIndex}`;
  ...
  public init(): void {
    ...
    if (this.isScanActiveCircuitSwitchOn) {
      ...
      if (this.isScanActiveCircuitLogicReady) {
        this.initActiveScanCircuitLogic();
      }
    } else {
  ...
  private setScanActiveCircuitSwitch(on: boolean): void {
    if (this.keyEventManager) {
      ...
      SimVar.SetSimVarValue(this.scanActiveCircuitSimVar!, SimVarValueType.Bool, on ? 1 : 0);
    }
  }
