import {
  ClockEvents, ComponentProps, ConsumerValue, DisplayComponent, EventBus, FSComponent, GPSSatComputerEvents, GPSSystemState,
  LNavDataEvents, LNavUtils, MathUtils, NavMath, NodeReference, NumberFormatter, Subject, VNode
...
export abstract class ArcCdi extends DisplayComponent<ArcCdiProps> {
  ...
  private readonly xtkSource = ConsumerValue.create(null, 0);
  ...
  public onAfterRender(): void {
    ...
    const sub = this.props.bus.getSubscriber<ClockEvents & LNavDataEvents & GPSSatComputerEvents>();
   ...
   */
  private onXtkChanged(): void {
    const xtk = MathUtils.round(this.xtkSource.get(), 0.001);
    ...
    this.currentXtk = xtk;
    this.xtkLabel.set(this.xtkLabelFormatter(Math.abs(MathUtils.round(xtk, 0.1))));
  }
