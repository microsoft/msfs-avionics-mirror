import { MappedSubscribable, MathUtils } from '@microsoft/msfs-sdk';
...
export class FmsSpeedsConfig implements Config {
  ...
  private parseAirframeLimits(element: Element | null): FmsAirframeSpeedLimits {
    const ias = element?.querySelector(':scope>Ias');
    const mach = element?.querySelector(':scope>Mach');

    return {
      ias: new FmsAirframeSpeedLimitConfig(ias ?? this.generalLimits.maximumIas),
      mach: new FmsAirframeSpeedLimitConfig(mach ?? this.generalLimits.maximumMach)
    };
  }
...
export class FmsAirframeSpeedLimitConfig implements NumericConfig {
  ...
  public readonly value: number | LookupTableConfig;
   ...
   */
  constructor(source: Element | number) {
  ...
  /** @inheritdoc */
  public resolve(): (context: FmsAirframeSpeedLimitContext) => number | MappedSubscribable<number> {
