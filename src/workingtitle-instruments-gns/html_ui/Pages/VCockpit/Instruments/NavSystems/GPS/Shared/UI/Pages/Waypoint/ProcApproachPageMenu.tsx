 ...
 */
export class ProcApproachPageMenu extends MenuDefinition {
  ...
  public readonly entries: readonly MenuEntry[] = [
    ...
    {
      label: 'Load And Activate?', disabled: this.noProcedureLoaded, action: (): void => {
        const airport = this.findNextAirportInPlan();
        if (airport !== undefined) {
    ...
    {
      label: 'Select Approach Chnl?', disabled: true, action: (): void => {

      }
    },
    {
      label: 'Select Next FPL Apt?', disabled: this.hasNoDestinationAirport, action: (): void => {
  ...
  constructor(
    ...
    private insertApproach: (activate: boolean) => Promise<void>
  ) {
    super();
