...
import {
  AhrsEvents, APLateralModes, APValues, APVerticalModes, ControlEvents, EventBus, NavComEvents, NavEvents, NavMath, NavRadioIndex, NavSourceId, NavSourceType,
  NavToNavManager2, Subject
} from '@microsoft/msfs-sdk';

import {
  CDIScaleLabel, FgpUserSettings, FMS_MESSAGE_ID, MessageService, VorTuningMode, WT21ControlEvents, WT21FmsUtils, WT21GhostNeedleControlEvents,
  WT21LNavDataEvents
} from '@microsoft/msfs-wt21-shared';

/** NavToNav manager using the state pattern */
export class WT21NavToNavManager implements NavToNavManager2 {
  /** @inheritDoc */
  ...
  public getArmableLateralMode(): number {
    return this.currentState.canLocArm() ? APLateralModes.LOC : APLateralModes.NONE;
  }

  /** @inheritDoc */
  public getArmableVerticalMode(): number {
    return this.currentState.canLocArm() ? APVerticalModes.GS : APVerticalModes.NONE;
  }
...
class NavToNavArmed extends NavToNavState {
  ...
  private readonly lateralArmedSub = this.navToNav.apValues.lateralArmed.sub(this.tryGoToNextState, false, true);
...
class NavToNavTransferred extends NavToNavState {
  ...
  private readonly cdiSub = this.bus.getSubscriber<NavEvents>().on('cdi_select').handle(this.handleCdiSelect);
