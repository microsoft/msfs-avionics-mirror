import { ConsumerSubject, ControlEvents, DebounceTimer, ElectricalEvents, MappedSubject, NavComSimVars, NumberFormatter, XPDRSimVarEvents } from '@microsoft/msfs-sdk';
...
import { ComFrequencyFormat, Formatter, NavFrequencyFormat, TransponderCodeFormat, Validator } from '../Framework/FmcFormats';
 ...
 * TUNE apge
 */
export class TunePage extends FmcPage {
  ...
  private readonly fgpSettings = FgpUserSettings.getManager(this.eventBus);
  ...
  private readonly nav1ActiveData = new DataInterface(MappedSubject.create(([freq, mode]) => {
    ...
    ConsumerSubject.create(this.eventBus.getSubscriber<NavComSimVars>().on('nav_active_frequency_1').whenChanged(), 0),
  ...
  private readonly nav2ActiveData = new DataInterface(MappedSubject.create(([freq, mode]) => {
    ...
    ConsumerSubject.create(this.eventBus.getSubscriber<NavComSimVars>().on('nav_active_frequency_2').whenChanged(), 0),
  ...
  private readonly com1Field = new TextInputField(this, {
    ...
    deleteAllowed: false,
    style: '[green]',
  }).bindConsumer(this.eventBus.getSubscriber<NavComSimVars>().on('com_active_frequency_1').whenChanged(),
  ...
  private readonly com2Field = new TextInputField(this, {
    ...
    deleteAllowed: false,
    style: '[green]',
  }).bindConsumer(this.eventBus.getSubscriber<NavComSimVars>().on('com_active_frequency_2').whenChanged(),
  ...
  }).bindConsumer(this.eventBus.getSubscriber<NavComSimVars>().on('com_standby_frequency_1').whenChanged(),
  ...
  }).bindConsumer(this.eventBus.getSubscriber<NavComSimVars>().on('com_standby_frequency_2').whenChanged(),
  ...
  }).bindConsumer(this.eventBus.getSubscriber<XPDRSimVarEvents>().on('xpdr_code_1').whenChanged(),
    (value) => { this.eventBus.getPublisher<ControlEvents>().pub('publish_xpdr_code_1', value); });

  private readonly adfFreqField = new TextInputField(this, {
    formatter: new AdfFrequencyFormat(),
    deleteAllowed: false,
    style: '[green]',
  }).bindConsumer(this.eventBus.getSubscriber<NavComSimVars>().on('adf_active_frequency_1').whenChanged(),
  ...
  protected onInit(): void {
    const sub = this.eventBus.getSubscriber<ElectricalEvents>();
