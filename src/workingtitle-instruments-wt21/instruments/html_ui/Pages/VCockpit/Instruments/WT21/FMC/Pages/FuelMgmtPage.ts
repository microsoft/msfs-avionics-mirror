import {
  ClockEvents, DataInterface, DisplayField, DurationFormatter, EngineEvents, Facility, FlightPlanCopiedEvent, FlightPlannerEvents, FmcRenderTemplate, Formatter,
  GeoPoint, GNSSEvents, ICAO, LineSelectKeyEvent, MutableSubscribable, PageLinkField, Subject, TextInputField, UnitType
} from '@microsoft/msfs-sdk';

import { DefaultsUserSettings, WT21FmsUtils, WT21UnitsUtils } from '@microsoft/msfs-wt21-shared';

import { ConvertableNumberAndUnitFormat, NumberAndUnitFormat, SimpleStringFormat, StringInputFormat } from '../Framework/FmcFormats';
 ...
 */
export class FuelMgmtPage extends WT21FmcPage {

  private readonly perfMenuLink = PageLinkField.createLink(this, 'PERF MENU>', '/perf-menu');

  private readonly engEventsSub = this.bus.getSubscriber<EngineEvents>();
  private readonly clockEventsSub = this.bus.getSubscriber<ClockEvents>();
  private readonly gnssEventsSub = this.bus.getSubscriber<GNSSEvents>();

  private planOrigin: string | undefined;
  private readonly flightPlanCopiedConsumer = this.bus.getSubscriber<FlightPlannerEvents>().on('fplCopied');
  ...
  private readonly defaultsSettings = DefaultsUserSettings.getManager(this.bus);
  ...
  }).bind(this.fms.performancePlanProxy.reserveFuel as MutableSubscribable<number>); // FIXME wt21 remove this cast (rework perf plan)
   ...
   */
  private async setFacility(isTo: boolean, ident: string | null): Promise<boolean> {

    if (ident !== null && ident.length > 0) {

      const facility = await this.screen.selectWptFromIdent(ident, this.fms.ppos);
  ...
  protected onInit(): void {
    ...
    // new flight plan
    this.addBinding(this.flightPlanCopiedConsumer.handle((evt: FlightPlanCopiedEvent) => {
  ...
  protected async onHandleSelectKey(event: LineSelectKeyEvent): Promise<boolean | string> {
    if (event.col === 1 && event.row === (6 * 2)) {
      this.screen.navigateTo('/perf-menu');
      return true;
    }
