import {
  BasicNavAngleSubject, BasicNavAngleUnit, Facility, FSComponent, GeoPoint, ICAO,
  NearestSubscription, NumberFormatter, NumberUnitSubject, Subject, Subscribable, UnitType, VNode,
} from '@microsoft/msfs-sdk';

import { AirportWaypoint, GarminFacilityWaypointCache } from '@microsoft/msfs-garminsdk';

import { G1000WaypointIcon } from '../../../../Shared/UI/Waypoint';
...
export class FacilityItem<T extends Facility> extends G1000UiControl<FacilityItemProps<T>> {
  ...
  private readonly distance = NumberUnitSubject.create(UnitType.GA_RADIAN.createNumber(NaN));
  ...
  private readonly waypointIcon = FSComponent.createRef<G1000WaypointIcon>();
  ...
  /** @inheritdoc */
  public render(): VNode {
    const waypoint = this.props.facWaypointCache.get(this.props.facility);
    ...
    return (
      <div class='mfd-nearest-facility'>
        ...
        <G1000WaypointIcon
          ref={this.waypointIcon}
          waypoint={Subject.create(waypoint)}
          planeHeading={waypoint instanceof AirportWaypoint ? this.props.planeHeading : undefined}
          class='mfd-nearest-facility-icon'
        />
        <BearingDisplay
          value={this.bearing}
  ...
  public destroy(): void {
    super.destroy();
    this.waypointIcon.getOrDefault()?.destroy();
  }
}