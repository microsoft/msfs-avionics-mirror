import {
  AdcEvents, AhrsEvents, AirportFacility, BaseLegPredictions, BitFlags, ClockEvents, DurationFormatter, EngineEvents, FacilityType,
  FlightPlannerEvents, FlightPlanPredictor, GNSSEvents, ICAO, LegDefinitionFlags, LegType, MappedSubject, Subject, UnitType,
} from '@microsoft/msfs-sdk';

import { WT21Fms } from '../../Shared/FlightPlan/WT21Fms';
import { WT21AlternatePredictor } from '../../Shared/Navigation/WT21AlternatePredictor';
...
import { Formatter, NumberAndUnitFormat } from '../Framework/FmcFormats';
 ...
 * Progress page
 */
export class ProgPage extends FmcPage {
  // Data
  ...
  private lnavDtg: number | null = null;

  private lnavDestDtg: number | null = null;
  ...
  private readonly WindField = new DisplayField<readonly [number, number] | null>(this, {
    formatter: {
      ...
      /** @inheritDoc */
      format([direction, speed]: [number, number]): string {
  ...
  private readonly SatIsaDevField = new DisplayField<readonly [number, number] | null>(this, {
    formatter: {
      ...
      /** @inheritDoc */
      format([sat, isaDev]: [number, number]): string {
  ...
  protected onInit(): void {
    ...
    this.addBinding(this.eventBus.getSubscriber<GNSSEvents>().on('zulu_time').atFrequency(this.PREDICTIONS_UPDATE_RATE_HZ).handle((value) => {
      this.currentUtcSeconds = value;
    }));

    this.addBinding(this.eventBus.getSubscriber<GNSSEvents>().on('ground_speed').atFrequency(this.PREDICTIONS_UPDATE_RATE_HZ).handle((value) => {
      this.currentGroundSpeed = Math.max(150, value);
    }));

    this.addBinding(this.eventBus.getSubscriber<EngineEvents>().on('fuel_total').atFrequency(this.PREDICTIONS_UPDATE_RATE_HZ).handle((value) => {
      this.currentFuelTotalQuantity = value;
    }));

    this.addBinding(this.eventBus.getSubscriber<EngineEvents>().on('fuel_flow_total').atFrequency(this.PREDICTIONS_UPDATE_RATE_HZ).handle((value) => {
      this.currentFuelFlow = value;
    }));

    this.addBinding(this.eventBus.getSubscriber<EngineEvents>().on('fuel_weight_per_gallon').atFrequency(this.PREDICTIONS_UPDATE_RATE_HZ).handle((value) => {
      return this.currentFuelWeightPerGallon = value;
    }));

    this.addBinding(this.eventBus.getSubscriber<AhrsEvents>().on('hdg_deg_true').atFrequency(this.WIND_UPDATE_RATE_HZ).handle((heading) => {
      this.HeadingSubject.set(heading);
    }));

    this.addBinding(this.eventBus.getSubscriber<AdcEvents>().on('ambient_wind_direction').atFrequency(this.WIND_UPDATE_RATE_HZ).handle((value) => {
      this.WindDirectionSubject.set(value);
    }));

    this.addBinding(this.eventBus.getSubscriber<AdcEvents>().on('ambient_wind_velocity').atFrequency(this.WIND_UPDATE_RATE_HZ).handle((value) => {
      this.WindSpeedSubject.set(value);
    }));

    this.addBinding(this.eventBus.getSubscriber<AdcEvents>().on('ambient_temp_c').atFrequency(1).handle((value) => {
      this.SatSubject.set(value);
    }));

    this.addBinding(this.eventBus.getSubscriber<AdcEvents>().on('isa_temp_c').atFrequency(1).handle((value) => {
      this.IsaTempSubject.set(value);
    }));

    this.addBinding(this.eventBus.getSubscriber<AdcEvents>().on('tas').atFrequency(1).handle((value) => {
      this.Tas.set(value);
    }));

    this.addBinding(this.eventBus.getSubscriber<WT21LNavDataEvents>().on('lnavdata_waypoint_distance').atFrequency(1).handle((value) => {
      return this.lnavDtg = value;
    }));

    this.addBinding(this.eventBus.getSubscriber<WT21LNavDataEvents>().on('lnavdata_destination_distance').atFrequency(1).handle((value) => {
      return this.lnavDestDtg = value;
    }));

    this.addBinding(this.eventBus.getSubscriber<WT21LNavDataEvents>().on('lnavdata_xtk').atFrequency(1).handle((value) => {
      this.Xtk.set(value);
    }));
    ...
    this.addBinding(this.eventBus.getSubscriber<WT21LNavDataEvents>().on('lnavdata_nominal_leg_index').whenChanged().handle((index) => {
    ...
    this.addBinding(this.eventBus.getSubscriber<FlightPlannerEvents>().on('fplCopied').handle(() => {
      this.handleFlightPlanChange();
      this.handlePeriodicallyRecomputePredictions();
    }));

    this.addBinding((this.eventBus.getSubscriber<ClockEvents>().on('simTime').handle(() => {
    ...
    this.addBinding(this.eventBus.getSubscriber<ClockEvents>().on('simTime').atFrequency(this.PREDICTIONS_UPDATE_RATE_HZ).handle(() => {
      this.handlePeriodicallyRecomputePredictions();
    }));
