import {
  AdcEvents, AhrsEvents, AirportFacility, BaseLegPredictions, BitFlags, ClockEvents, DisplayField, DurationFormatter, EngineEvents, FacilityType,
...
export class ProgPage extends WT21FmcPage {
  ...
  protected onInit(): void {
    ...
    this.addBinding(this.bus.getSubscriber<WT21LNavDataEvents>().on('lnavdata_waypoint_distance').atFrequency(1).handle((value) => {
      return this.lnavDtg = value;
    }));

    this.addBinding(this.bus.getSubscriber<WT21LNavDataEvents>().on('lnavdata_destination_distance').atFrequency(1).handle((value) => {
      return this.lnavDestDtg = value;
    }));

    this.addBinding(this.bus.getSubscriber<WT21LNavDataEvents>().on('lnavdata_xtk').atFrequency(1).handle((value) => {
      this.Xtk.set(value);
    }));
