...
import { ImgTouchButtonProps, TouchButton, TouchButtonHoldAction, TouchButtonHoldEndReason, TouchButtonOnTouchedAction } from '@microsoft/msfs-garminsdk';
 ...
 */
export interface UiImgTouchButtonProps
  extends Omit<ImgTouchButtonProps, 'onTouched' | 'onPressed' | 'onHoldStarted' | 'onHoldTick' | 'onHoldEnded' | 'focusOnDrag' | 'class'> {
 ...
 * focus.
 */
export class UiImgTouchButton extends DisplayComponent<UiImgTouchButtonProps> implements UiFocusableComponent {
  /** @inheritdoc */
  public readonly isUiFocusableComponent = true;

  protected readonly buttonRef = FSComponent.createRef<ImgTouchButton>();
  ...
  protected onTouched(): TouchButtonOnTouchedAction {
    if (this.props.onTouched) {
      return this.props.onTouched(this);
    } else {
  ...
  protected onPressed(button: TouchButton, isHeld: boolean): void {
    this.props.onPressed && this.props.onPressed(this, isHeld);
  }
  ...
  protected onHoldTick(button: TouchButton, dt: number, totalTime: number, timeSinceLastPress: number): TouchButtonHoldAction {
    return this.props.onHoldTick
      ? this.props.onHoldTick(this, dt, totalTime, timeSinceLastPress)
      : TouchButtonHoldAction.None;
  }
  ...
  protected onHoldEnded(button: TouchButton, totalHoldDuration: number, endReason: TouchButtonHoldEndReason): void {
    this.props.onHoldEnded && this.props.onHoldEnded(this, totalHoldDuration, endReason);
  }
