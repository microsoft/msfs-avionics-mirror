import { DmsFormatter, Facility, MappedSubject } from '@microsoft/msfs-sdk';
...
import { Formatter, Validator } from '../../Framework/FmcFormats';
...
export class FixInfoPage extends FmcPage {
  ...
  protected readonly controller = new WT21FixInfoPageController(this.eventBus, this.fms, this.store, this, this.pageManager);
  ...
  /** @inheritDoc */
  public override handleScrollKey(event: FmcPrevNextEvent): boolean {
    if (event === FmcPrevNextEvent.BTN_PREV) {
      this.controller.previous();
      return true;
    } else if (event === FmcPrevNextEvent.BTN_NEXT) {
  ...
  // 1L - ref fix
  protected readonly FixField = new TextInputField<string, Facility>(this, {
    formatter: {
      nullValueString: '-----',
  ...
  protected readonly AbeamField = new DisplayField<string | null>(this, {
    formatter: {
      nullValueString: '<ABEAM REF',
      /** @inheritDoc */
      format() { return this.nullValueString; },
    },
    /** @inheritdoc */
    onSelected: async () => {
      if (this.store.currentFixIdent.get() === null) {
