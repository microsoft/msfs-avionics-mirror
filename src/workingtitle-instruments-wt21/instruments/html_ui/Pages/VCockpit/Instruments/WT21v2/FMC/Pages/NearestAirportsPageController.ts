...
export class NearestAirportsPageController {
  ...
  private nrstSearchSession?: NearestAirportSearchSession<NearestIcaoSearchSessionDataType.Struct>;
  ...
  constructor(
    ...
    this.fms.facLoader.startNearestSearchSessionWithIcaoStructs(FacilitySearchType.Airport).then(session => {
  ...
  private async doUpdate(): Promise<void> {
    ...
    this.nearestAirports = this.nearestAirports.filter(airport => !results.removed.find(icao => ICAO.valueEquals(icao, airport.icaoStruct)));

    const newAirports = await this.fms.facLoader.getFacilitiesOfType(FacilityType.Airport, results.added);
