...
import { Epic2LNavDataEvents } from '../Epic2LNavDataEvents';
...
export class GpsSource<T extends readonly string[]> extends NavSourceBase<T> {
  ...
  // private readonly advisoryVnavSetting = FmcUserSettings.getManager(this.bus).getSetting('advisoryVnavEnabled');
  private readonly advisoryVnavSetting = { value: true };
  ...
  public constructor(
    ...
    const lnav = this.bus.getSubscriber<LNavEvents & Epic2LNavDataEvents>();
    ...
    lnav.on('lnavdata_flight_area').whenChanged().handle(this.setters.get('lateralDeviationScalingLabel')!.setter);
    lnav.on('lnavdata_nominal_leg_index').whenChanged().handle(this.handleEffectiveLegIndex);

    this.lnavIsTracking.pipe(this.signalStrength, isTracking => isTracking ? 1 : 0);
