...
import {
  AdcSystemEvents, AdcSystemSelectorEvents, PerformancePlan, PFDSettings, PFDUserSettings, RASystemEvents, WT21ControlEvents
...
export class Altimeter extends DisplayComponent<AltimeterProps> {
  ...
  public onAfterRender(): void {
    ...
    this.selectedAdc.sub((adcIndex) => {
      ...
      this.adcSubs.push(
        adc.on(`adc_state_${adcIndex}`).whenChanged().handle(this.onAdcStateChanged.bind(this)),
        adc.on(`adc_indicated_alt_${adcIndex}`).withPrecision(1).handle(this.updateAltitude.bind(this)),
        adc.on(`adc_radio_alt_${adcIndex}`).withPrecision(1).handle(this.updateGroundRibbon.bind(this)),
  ...
  private onAdcStateChanged(state: AvionicsSystemStateEvent): void {
    if (state.current === AvionicsSystemState.On && state.previous === AvionicsSystemState.Initializing) {
      ...
      this.altOffset = 0;
      this.initAltOffset = 0;
      this.updateAltitude(this.altitude);
    }
  }

  /**
   * A callback called when the ADC altitude validity changes.
   * @param valid Is the altitude data valid?
   */
  private onAltitudeDataValidityChanged(valid: boolean): void {
    this.altimeterContainerRef.instance.classList.toggle('fail', !valid);
  }
