import {
  AirportFacility, DmsFormatter, FacilitySearchType, Formatter, ICAO, LatLongInterface, NumberFormatter, OneWayRunway, Subscribable, Unit, UnitType, Validator,
  WindEntry
...
export const TimeFormatter: Formatter<Date> = {
  ...
  format(value: Date): string {
    return value.getTime() === 0 ? '--:--' : `${pad(value.getUTCHours())}:${pad(value.getUTCMinutes())}`;
  },
};
