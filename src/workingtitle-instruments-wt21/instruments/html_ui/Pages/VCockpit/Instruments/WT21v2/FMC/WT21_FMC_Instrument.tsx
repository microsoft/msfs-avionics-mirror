...
import { WT21APUtils } from './Autopilot/WT21APUtils';
...
export class WT21_FMC_Instrument implements FsInstrument {
  ...
  private readonly flightPlanRouteSyncManager?: WT21FlightPlanRouteSyncManager;
  ...
  constructor(readonly instrument: BaseInstrument) {
    ...
    this.lnavComputer = new LNavComputer(
      ...
      {
        maxBankAngle: 35,
        intercept: WT21APUtils.lnavInterceptCurve,
    ...
    if (this.instrument.isPrimary) {
      this.flightPlanRouteSyncManager = new WT21FlightPlanRouteSyncManager(this.fms);
  ...
  private initializeAutopilot(): void {
    ...
    const apConfig = new WT21APConfig(
      this.bus,
      this.planner,
      this.messageService,
  ...
  private async initFlightPlan(): Promise<void> {
    if (this.instrument.isPrimary) {
      ...
      this.flightPlanRouteSyncManager!.init(routeManager, new WT21FlightPlanRouteLoader(this.fms));
      ...
      if (!FlightPlanRouteUtils.isRouteEmpty(routeToLoad, false)) {
        await this.flightPlanRouteSyncManager!.loadRoute(routeToLoad, true);
      }
      ...
      await this.syncPrimaryFlightPlan();
    }
  }
  ...
  /**
   * Syncs the primary flight plan from another instrument.
   */
  private async syncPrimaryFlightPlan(): Promise<void> {
    this.fms.flightPlanner.requestSync();
