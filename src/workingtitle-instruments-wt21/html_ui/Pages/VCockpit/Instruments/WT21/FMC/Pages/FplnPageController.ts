import {
  AirportFacility, AirwayObject, BitFlags, ComputedSubject, ConsumerSubject, EventBus, FacilitySearchType, FacilityType, FixTypeFlags, FlightPathUtils, FlightPlanActiveLegEvent,
  FlightPlanCalculatedEvent, FlightPlanCopiedEvent, FlightPlanLegEvent, FlightPlannerEvents, FlightPlanSegmentType, FlightPlanUserDataEvent, GeoPoint, GNSSEvents, ICAO,
  LegDefinition, LegType, Subject, UnitType,
...
export class FplnPageController {
  ...
  public readonly altnField = new TextInputField<string | null>(this.page, {
    ...
    onSelected: async (scratchpadContents): Promise<string | boolean> => {
      ...
      const facility = await this.pageManager.selectWptFromIdent(scratchpadContents, this.fms.ppos, FacilitySearchType.Airport);
   ...
   */
  public renderRow = (page: FmcPage, data: RoutePageLegItem | undefined): FmcRenderTemplateRow[] => {
    if (data !== undefined) {
  ...
  private airwayInEdit: AirwayObject | undefined;
  ...
  private getData(): void {
    if (this.fms.hasFlightPlan(this.fms.getPlanIndexForFmcPage())) {
      ...
      this.store.altn.set(plan.getUserData('wt21.altn') ? ICAO.getIdent(plan.getUserData(WT21Fms.USER_DATA_KEY_ALTN) as string) : null);
  ...
  public async setOrigin(ident: string | null): Promise<void> {
    if (!Simplane.getIsGrounded()) {
      this.page.showScratchpadError('NOT ON GROUND');
      return;
    }

    if (!this.fms.canEditPlan()) {
      this.page.showScratchpadError('XSIDE EDIT IN PROGRESS');
      return;
    }
    ...
    this.page.showScratchpadError('FACILITY NOT FOUND');
  }
