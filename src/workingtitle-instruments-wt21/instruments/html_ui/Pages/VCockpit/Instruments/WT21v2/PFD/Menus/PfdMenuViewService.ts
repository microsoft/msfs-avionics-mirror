...
import {
  DcpEvent, DcpEvents, DisplayUnitConfigInterface, DisplayUnitLayout, GuiHEvent, MenuViewService, WT21CourseNeedleNavIndicator
...
const WT21_H_EVENT_CCP_REGEX = /Generic_Upr_([12])_(.*)/;
const WT21_H_EVENT_DISPLAY_REGEX = /Generic_Display_PFD([12])_(.*)/;
...
export class PfdMenuViewService extends MenuViewService<PfdMenuViewKeys> {
  ...
  public onInteractionEvent(hEvent: string, instrumentIndex: number): boolean {
    ...
    if (hEvent.startsWith('Generic_Upr')) {
      const hEventSplit = WT21_H_EVENT_CCP_REGEX[Symbol.match](hEvent);

      if (hEventSplit === null) {
        return false;
      }

      const eventInstrIndex = Number(hEventSplit[1]);
      const evt = this.guiEventMap.get(hEventSplit[2]);

      if (evt !== undefined && eventInstrIndex === instrumentIndex) {
      ...
      const hEventSplit = WT21_H_EVENT_DISPLAY_REGEX[Symbol.match](hEvent);
