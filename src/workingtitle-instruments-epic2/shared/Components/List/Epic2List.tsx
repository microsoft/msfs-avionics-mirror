...
import {
  ComponentProps, DebounceTimer, DisplayComponent, EventBus, FSComponent, MappedSubject, SetSubject, Subject, Subscribable, SubscribableArray, SubscribableSet,
  SubscribableUtils, Subscription, VNode
} from '@microsoft/msfs-sdk';

import { DynamicList } from './DynamicList';
import { DynamicListData } from './DynamicListData';
import { Epic2TouchListProps } from './Epic2TouchList';
...
import './Epic2List.css';

/**
 * Formatting props for Epic2List.
 */
export type Epic2ListFormattingProps = Omit<Epic2TouchListProps, 'itemCount' | 'maxRenderedItemCount'>;

/**
 * Component props for Epic2List.
 */
export interface Epic2ListProps<DataType> extends ComponentProps, Epic2ListFormattingProps {
  /** The event bus. */
  bus: EventBus,

  /** 'inside' puts the scrollbar inside the list on the right side, 'outside' moves it outside the list to the right. */
  scrollbarStyle: 'inside' | 'outside';
 ...
 */
export class Epic2List<DataType extends DynamicListData> extends DisplayComponent<Epic2ListProps<DataType>> /*implements GtcInteractionHandler*/ {
  private readonly Epic2ListRef = FSComponent.createRef<HTMLDivElement>();
  private readonly scrollBarContainerRef = FSComponent.createRef<HTMLDivElement>();
  ...
  private readonly isVisible = SubscribableUtils.toSubscribable(this.props.isVisible ?? true, true);
  ...
  public onAfterRender(): void {
    ...
    canScroll.sub(val => {
      this.scrollBarContainerRef.instance.style.opacity = val ? '1' : '0';
    }, true);
    ...
    this.isVisible.sub(isVisible => {
      this.Epic2ListRef.instance.classList.toggle('hidden', !isVisible);
    }, true);
  }
  ...
  private updateScrollBarTranslation(): void {
    // Add 4 to account for the scroll bar container overlapping the list border.
    const heightPx = this.touchListRef.instance.lengthPx.get() + 4;
  ...
  private updateRenderedWrappers(): void {
    ...
    this.dynamicList.forEachComponent<Epic2ListItemWrapper>((wrapper, index) => {
  ...
  /** @inheritdoc */
  public render(): VNode {
    const scrollbarStyleClass = this.props.scrollbarStyle === 'inside'
      ? 'epic2-list-scroll-style-inside'
      : 'epic2-list-scroll-style-outside';

    let cssClass: string | SetSubject<string>;

    if (typeof this.props.class === 'object') {
      cssClass = SetSubject.create(['epic2-list', scrollbarStyleClass]);
      this.cssClassSub = FSComponent.bindCssClassSet(cssClass, this.props.class, ['epic2-list', scrollbarStyleClass]);
    } else if (this.props.class !== undefined) {
      cssClass = [
        'epic2-list',
        scrollbarStyleClass,
        ...(FSComponent.parseCssClassesFromString(this.props.class)
          .filter(classToAdd => classToAdd !== 'epic2-list'))
      ].join(' ');
    } else {
      cssClass = 'epic2-list ' + scrollbarStyleClass;
    }

    return (
      <div ref={this.Epic2ListRef} class={cssClass}>
        <TouchList
          ...
          itemCount={this.props.itemCount ?? this.visibleItemCount}
          bus={this.props.bus}
        >
          {this.props.staticTouchListChildren}
        </TouchList>
        <div ref={this.scrollBarContainerRef} class="epic2-list-scroll-bar-container">
          <div ref={this.scrollBarRef} class="epic2-list-scroll-bar" />
        </div>
      </div>
    );
  }
 ...
 */
class Epic2ListItemWrapper extends DisplayComponent<ComponentProps> {
  private thisNode?: VNode;

  private readonly cssClass = SetSubject.create(['epic2-list-item-wrapper']);
