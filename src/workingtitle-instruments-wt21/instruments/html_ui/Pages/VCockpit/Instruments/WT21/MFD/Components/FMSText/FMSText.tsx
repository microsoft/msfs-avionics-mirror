import {
  AirportFacility, AltitudeRestrictionType, BitFlags, ClockEvents, ComponentProps, ConsumerSubject, DateTimeFormatter, DisplayComponent, DurationFormatter, EngineEvents, EventBus,
  FacilityLoader, FacilityType, FlightPlanner, FlightPlanPredictor, FSComponent, GNSSEvents, ICAO, LegDefinition, LegPredictions, LegType, LNavEvents, MathUtils, NumberFormatter,
  Subject, Subscribable, SubscribableMapFunctions, UnitType, VerticalData, VerticalFlightPhase, VNavEvents, VNavUtils, VNode,
} from '@microsoft/msfs-sdk';

import {
  MapUserSettings, MFDUserSettings, PerformancePlanProxy, WT21_PFD_MFD_Colors, WT21FlightPlanPredictorConfiguration, WT21FmsUtils, WT21LegDefinitionFlags, WT21LNavDataEvents,
  WT21UnitsUtils, WT21VNavDataEvents,
...
export class FMSText extends DisplayComponent<FMSTextProps> {
  ...
  private readonly vnavShowSetting = this.mfdSettingManager.getSetting('mfdUpperFmsTextVNavShow');
  ...
  private readonly updateConsumer = this.props.bus.getSubscriber<ClockEvents>().on('simTime').atFrequency(FMSText.UPDATE_FREQ);
  ...
  public onAfterRender(): void {
    const sub = this.props.bus.getSubscriber<GNSSEvents & EngineEvents & LNavEvents & WT21LNavDataEvents>();
    ...
    this.lnavIsTracking.setConsumer(sub.on('lnav_is_tracking').atFrequency(1));
    ...
    this.lnavNominalLegIndex.setConsumer(sub.on('lnavdata_nominal_leg_index').atFrequency(1));

    this.mfdSettingManager.whenSettingChanged('mfdUpperFmsTextVNavShow').handle(show => {
      this.vnavRef.instance.setVisibility(this.isVisible && show);
    });
    this.mapSettingManager.whenSettingChanged('hsiFormat').handle(format => {
  ...
  private applyVisibilityState(): void {
    ...
    if (this.isVisible && this.isCompatible) {
      ...
      this.updateConsumer.handle(this.updateHandler);
    } else {
      this.ppos.pause();
      ...
      this.updateConsumer.off(this.updateHandler);
    }
...
class FMSTextVNavWindow extends DisplayComponent<FMSTextVnavWindowProps> {
  ...
  private readonly distanceToActiveLegEnd = ConsumerSubject.create(null, NaN).pause();
  private readonly lnavNominalLegIndex = ConsumerSubject.create(null, -1).pause();
  ...
  public onAfterRender(): void {
    ...
    const sub = this.props.bus.getSubscriber<GNSSEvents & WT21LNavDataEvents & VNavEvents & WT21VNavDataEvents>();

    this.ppos.setConsumer(sub.on('gps-position').atFrequency(1));
    this.groundSpeed.setConsumer(sub.on('ground_speed').atFrequency(1));

    this.distanceToActiveLegEnd.setConsumer(sub.on('lnavdata_tracked_leg_end_distance').atFrequency(1));
    this.lnavNominalLegIndex.setConsumer(sub.on('lnavdata_nominal_leg_index').atFrequency(1));
  ...
  private applyVisibilityState(): void {
    ...
    if (this.isVisible) {
      ...
      this.distanceToActiveLegEnd.resume();
      this.lnavNominalLegIndex.resume();
      ...
      this.distanceToActiveLegEnd.pause();
      this.lnavNominalLegIndex.pause();
