...
/**
 * The base GNS530 instrument.
 */
class WT530 extends BaseInstrument {
  ...
  /**
   * Creates an instance of the WT530.
   */
  constructor() {
    super();
  ...
  protected Init(): void {
    ...
    SimVar.SetSimVarValue(`L:WT_GNS530_INSTALLED_${this.instrumentIndex}`, SimVarValueType.Bool, true);
  }
  ...
  private setOptions(): void {
    ...
    if (this.instrumentIndex === 1) {
      const as530NodeIndex = Array.from(elements).findIndex((node) => node.querySelector('Name')?.textContent === 'AS530');

      if (as530NodeIndex === -1) {
        nodeIndex = Array.from(elements).findIndex((node) => node.querySelector('Name')?.textContent === 'AS530_1');
      } else {
        nodeIndex = as530NodeIndex;
      }

    } else if (this.instrumentIndex > 1) {
      nodeIndex = Array.from(elements).findIndex((node) => node.querySelector('Name')?.textContent === `AS530_${this.instrumentIndex}`);
    }
...
function setup(): void {
  ...
  if (searchString.length > 0) {
        ...
        if (RegisterViewListener !== undefined && SimVar !== undefined && !isNavigating) {
          ...
          if (viewListener !== undefined) {
            ...
            if (!initted) {
              ...
              SimVar.SetSimVarValue(`L:WT_GNS530_INSTALLED_${instrumentIdx}`, SimVarValueType.Bool, false);
    ...
    registerInstrument('wt-gns530', WT530);
  }
}
