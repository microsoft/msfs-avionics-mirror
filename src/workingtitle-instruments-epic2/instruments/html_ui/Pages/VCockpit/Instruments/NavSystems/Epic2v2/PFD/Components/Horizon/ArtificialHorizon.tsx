import {
  BitFlags, ComponentProps, HorizonProjection, HorizonProjectionChangeType, HorizonSharedCanvasSubLayer, MathUtils, Subscribable, Subscription, Transform2D,
  Vec2Math
} from '@microsoft/msfs-sdk';

import './ArtificialHorizon.css';

// TODO clamp at extreme attitude
...
export interface ArtificalHorizonProps extends ComponentProps {
  ...
  isAttitudeValid: Subscribable<boolean>;
...
export class ArtificialHorizon extends HorizonSharedCanvasSubLayer<ArtificalHorizonProps> {
  public static readonly CLAMP_PITCH_MAX = 25;
  public static readonly CLAMP_PITCH_MIN = -10.8;
  ...
  public onAttached(): void {
    ...
    this.isValidSub = this.props.isAttitudeValid.sub(() => { this.needUpdate = true; }, true);
  ...
  public onUpdated(): void {
    ...
    if (this.props.show.get()) {
      ...
      // We clamp the pitch so that both the terrain and sky are always somewhat shown.
      const projection = this.projection;
      const pitchResolution = projection.getScaleFactor() / projection.getFov();
      const pitch = this.props.isAttitudeValid.get() ? MathUtils.clamp(projection.getPitch(), ArtificialHorizon.CLAMP_PITCH_MIN, ArtificialHorizon.CLAMP_PITCH_MAX) : 45;
      const roll = this.props.isAttitudeValid.get() ? projection.getRoll() : 0;
