...
export class GtcTakeoffDataPage extends GtcToldDataPage<ToldTakeoffPerformanceResult, GtcTakeoffDataPageProps> {
  ...
  public onOpen(): void {
    ...
    if (this.originDestIcaoSetting.value === '') {
      // When the onOpen() callback is called, the page is paused and the selected airport/runway subjects may not
      // reflect the value of the origin ICAO setting (the subjects are only synced while the page is resumed).
      // Therefore, we must reset the selected airport and runway subjects to null to ensure that the appropriate
      // handlers are notified if we succeed in auto-selecting an airport/runway.
