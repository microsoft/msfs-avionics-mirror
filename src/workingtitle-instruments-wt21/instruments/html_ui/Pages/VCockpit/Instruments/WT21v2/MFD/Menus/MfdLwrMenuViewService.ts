import { EventBus, Subject, UserSettingManager } from '@microsoft/msfs-sdk';

import {
  DisplayUnitLayout, GuiHEvent, InstrumentConfig, MapSettingsMfdAliased, MapUserSettings, MenuViewService, MFDSettingsAliased, MFDUpperWindowState,
...
export class MfdLwrMenuViewService extends MenuViewService<MfdLwrMenuViewKeys> {
  ...
  private readonly mfdLowerFormatSetting = this.mapSettingsManager.getSetting('hsiFormat');
  private readonly mfdUpperFormatSetting = this.mfdSettingManager.getSetting('mfdUpperWindowState');

  private mfdSoftkeyFormatChangeActiveTimeout: NodeJS.Timeout | undefined = undefined;
  private readonly isUsingSoftkeys = this.instrumentConfig.displayUnitConfig.displayUnitLayout === DisplayUnitLayout.Softkeys;
  private readonly formatChangeActiveSubject = Subject.create<boolean>(false);

  /**
   * MfdLwrMenuViewService constructor
   * @param bus the event bus
   * @param instrumentConfig The instrument config object
   * @param wt21ControlPublisher the WT21 control publisher
   * @param mapSettingsManager The map settings manager
   * @param mfdSettingManager The mfd settings manager
  ...
  public constructor(
    ...
    this.formatChangeActiveSubject.sub((isActive) => {
      this.wt21ControlPublisher.publishEvent(`softkeyFormatChangeActive_${this.instrumentConfig.instrumentIndex as 1 | 2}`, isActive);
    });
  }
