import {
  ...
  Metar, MetarWindSpeedUnits, NavMath, NumberFormatter, NumberUnitSubject, OneWayRunway, SetSubject, Subject,
  Subscribable, Subscription, Unit, UnitFamily, UnitType, UserSettingManager, VNode
} from '@microsoft/msfs-sdk';

import { AirportWaypoint, BearingDisplay, NumberUnitDisplay, UnitsUserSettingManager } from '@microsoft/msfs-garminsdk';

import { ControllableDisplayPaneIndex, PfdUserSettings, ToldRunwaySurfaceCondition, ToldUserSettingTypes } from '@microsoft/msfs-wtg3000-common';
...
export class GtcToldDataPageWeatherTab extends DisplayComponent<GtcToldDataPageWeatherTabProps> implements GtcToldDataPageTabContent {
  ...
  private readonly pressureDisplayUnit = PfdUserSettings.getAliasedManager(this.props.gtcService.bus, this.props.gtcService.pfdControlIndex)
    .getSetting('altimeterBaroMetric')
    .map(isMetric => isMetric ? UnitType.HPA : UnitType.IN_HG).pause();
  ...
  /** @inheritdoc */
  public onPause(): void {
    this.pressureDisplayUnit.pause();
  ...
  public onResume(): void {
    this.pressureDisplayUnit.resume();
  ...
  private async loadMetarData(): Promise<void> {
    ...
    if (metar.windSpeed !== undefined && metar.windSpeedUnits !== MetarWindSpeedUnits.Undefined && metar.windDir !== undefined) {
      ...
      this.windSpeedSetting.value = windSpeedUnit.convertTo(metar.windSpeed, UnitType.KNOT);
    }
  ...
  private async selectPressure(): Promise<void> {
    ...
    const isHpa = this.pressureDisplayUnit.get().equals(UnitType.HPA);
    ...
    const result = await this.props.gtcService.openPopup<GtcBaroPressureDialog>(GtcViewKeys.BaroPressureDialog1)
      .ref.request({
        ...
        unitsMode: isHpa ? 'hpa' : 'inhg',
        minimumValue: isHpa ? 948 : 28,
        maximumValue: isHpa ? 1084 : 32.01
      });
          ...
          {!this.props.isTakeoff && (
            ...
            >
              <div class='touch-button-value-value told-data-page-weather-baro-button-value'>
                <NumberUnitDisplay
                  value={this.pressure}
                  displayUnit={this.pressureDisplayUnit}
                  formatter={value => {
                    return this.pressureDisplayUnit.get().equals(UnitType.HPA)
                      ? GtcToldDataPageWeatherTab.HPA_FORMATTER(value)
                      : GtcToldDataPageWeatherTab.INHG_FORMATTER(value);
                  }}
  ...
  public destroy(): void {
    ...
    this.pressureDisplayUnit.destroy();
