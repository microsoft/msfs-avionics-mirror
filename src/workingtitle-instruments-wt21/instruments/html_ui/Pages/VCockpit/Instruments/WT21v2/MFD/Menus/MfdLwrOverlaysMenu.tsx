...
import {
  GuiDialog, GuiDialogProps, MapSettingsMfdAliased, MapUserSettings, PopupMenuSection, PopupSubMenu, RadioBox, RadioList
...
export class MfdLwrOverlaysMenu extends GuiDialog<MfdLwrOverlaysMenuProps> {
  ...
  private linkTerrainWxOption(): void {
    const terrWxSetting = this.props.mapSettingsManager.getSetting('terrWxState');
    this.terwxOption.sub(x => {
      terrWxSetting.value = MapUserSettings.terrWxStates[x];
    });
    this.props.mapSettingsManager.whenSettingChanged('terrWxState').handle(x => {
  ...
  private linkTFCOption(): void {
    const tfcEnabled = this.props.mapSettingsManager.getSetting('tfcEnabled');
    this.tfcOption.sub(x => {
      tfcEnabled.value = !!x;
    });
    this.props.mapSettingsManager.whenSettingChanged('tfcEnabled').handle(x => {
