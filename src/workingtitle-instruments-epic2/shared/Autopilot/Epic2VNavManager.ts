...
export class Epic2VNavManager implements VNavManager {
  ...
  private readonly flightArea = ConsumerSubject.create<Epic2FlightArea>(this.bus.getSubscriber<Epic2LNavDataEvents>().on('lnavdata_flight_area'), Epic2FlightArea.Departure);
  ...
  private readonly approachDetails = ConsumerSubject.create<Readonly<ApproachDetails>>(this.bus.getSubscriber<Epic2FmsEvents>().on('epic2_fms_approach_details_set'), {
  ...
  private cruiseAltitude = 20000;
  private cruiseAltitudeMetric = UnitType.METER.convertFrom(this.cruiseAltitude, UnitType.FOOT);
  ...
  private readonly tocBocDetailsSub = ObjectSubject.create<TocBocDetails>(Object.assign({}, this.tocBocDetails));

  private readonly grossWeight = MappedValue.create(
    ([bow, fuel, paxNum, paxWeight, cargo]): number =>
      bow + fuel + ((paxNum ?? 0) * paxWeight) + (cargo ?? 0),
    this.performancePlan.basicOperatingWeight,
    ConsumerSubject.create(this.bus.getSubscriber<WeightBalanceEvents>().on('total_weight'), 0),
    this.performancePlan.paxNumber,
    this.performancePlan.averagePassengerWeight,
    this.performancePlan.cargoWeight,
  );

  private readonly fmsMessageTransmitter = new FmsMessageTransmitter(this.bus);
  private readonly currentAltSubject = ConsumerSubject.create(this.bus.getSubscriber<AdcEvents>().on('indicated_alt'), 0);
  ...
  private readonly vnavFlightPhase = Subject.create<Epic2VerticalFlightPhase>(Epic2VerticalFlightPhase.None);

  private vtaActive = false;
  ...
  private readonly passingConstraintIn15Secs = Subject.create<boolean>(false);

  private readonly isAltConstraintViolated = MappedSubject.create(
    ([apVerticalMode, constraint, currentAltitude]) => {
      const currentAltMetre = UnitType.METER.convertFrom(currentAltitude, UnitType.FOOT);
      return constraint && (
        (apVerticalMode === APVerticalModes.FLC && currentAltMetre - constraint.maxAltitude > 45) ||
        (apVerticalMode === APVerticalModes.PATH && currentAltMetre - constraint.minAltitude < -45)
      );
    },
    this.apValues.verticalActive,
    this.nextConstraintDetails,
    this.currentAltSubject,
  );
   ...
   * @param performancePlan Perforance Plan
   ...
   * @param predictionFunctions An interface of the various aircraft specific vertical predictions
   */
  constructor(private readonly bus: EventBus,
    private readonly flightPlanner: FlightPlanner,
    public readonly calculator: VNavPathCalculator,
    private readonly performancePlan: Epic2PerformancePlan,
    private readonly apValues: APValues,
    private readonly primaryPlanIndex: number,
    private readonly facLoader: FacilityLoader,
    private readonly predictionFunctions: Epic2VerticalPredictionFunctions,
  ) {
    ...
    const lnavData = this.bus.getSubscriber<Epic2LNavDataEvents>();
    this.lnavDataCdiScaleLabel = ConsumerSubject.create(lnavData.on('lnavdata_flight_area'), 1);
    ...
    this.tocBocDetailsSub.sub(this.publishTOCBOCDetails.bind(this), true);
    ...
    this.performancePlan.cruiseAltitude.sub(v => {
      this.cruiseAltitude = v ?? 0;
      this.cruiseAltitudeMetric = UnitType.FOOT.convertTo(v ?? 0, UnitType.METER);
    });
      ...
      if (event.planIndex === Epic2FlightPlans.Active) {
        switch (event.type) {
          case OriginDestChangeType.OriginAdded:
            this.originFacility = event.airport ? await this.facLoader.getFacility(ICAO.getFacilityType(event.airport), event.airport) as AirportFacility : undefined;
            break;
          case OriginDestChangeType.OriginRemoved:
            this.originFacility = undefined;
            break;
          case OriginDestChangeType.DestinationAdded:
            this.destFacility = event.airport ? await this.facLoader.getFacility(ICAO.getFacilityType(event.airport), event.airport) as AirportFacility : undefined;
            break;
          case OriginDestChangeType.DestinationRemoved:
            this.destFacility = undefined;
            break;
        }
      }
    });

    this.vnavFlightPhase.sub((phase) => this.bus.getPublisher<Epic2FmsEvents>().pub('epic2_fms_vertical_flight_phase', phase));
  ...
  /** @inheritdoc */
  public tryDeactivate(newMode?: number): void {
    if (this.state !== VNavState.Disabled) {
   ...
   */
  private disarmPath(newMode: number = APVerticalModes.PITCH): void {
    if (this.pathMode.get() !== VNavPathMode.None) {
  ...
  public update(): void {
    ...
    if (lateralPlan.length > 0 && lateralLegIndex < lateralPlan.length && VNavUtils.verticalPlanHasLeg(verticalPlan, lateralLegIndex)) {
      ...
      const currentGroundSpeed = this.currentGroundSpeed;
      ...
      /*const tocBocDetails =*/ this.manageTocBocDetails(
          lateralPlan,
          verticalPlan,
          lateralLegIndex,
          alongLegDistance,
          currentGroundSpeed,
          currentAltitudeMetric,
          this.cruiseAltitudeMetric
        );
      ...
      const constraintPassingIn15Secs = currentConstraint !== undefined ?
        VNavUtils.getDistanceToConstraint(currentConstraint, lateralPlan, lateralLegIndex, alongLegDistance) >
        UnitType.NMILE.convertTo((this.currentGroundSpeed / 240), UnitType.METER) : false;
      ...
      this.passingConstraintIn15Secs.set(constraintPassingIn15Secs);
  ...
  private trackClimb(verticalPlan: VerticalFlightPlan, lateralPlan: FlightPlan): void {
    ...
    const minConstraintFt = UnitType.METER.convertTo(currentClimbConstraint?.minAltitude ?? 0, UnitType.FOOT);
    const maxConstraintFt = UnitType.METER.convertTo(currentClimbConstraint?.maxAltitude ?? 0, UnitType.FOOT);
    const targetAltitude = currentClimbConstraint ? MathUtils.clamp(selectedAltitude, minConstraintFt, maxConstraintFt) : Number.POSITIVE_INFINITY;

    this.targetAltitude.set(currentClimbConstraint !== undefined ? targetAltitude : null);

    if (this.awaitingClimbRearm &&
      (currentClimbConstraint === undefined || Math.round(targetAltitude) > this.capturedAltitude)) {
    ...
    if (!this.isAltCaptured && this.apValues.verticalActive.get() !== APVerticalModes.ALT) {
      const nonPathDeviation = targetAltitude - this.currentAltitude;
  ...
  private isXtkOutsideLimits(): boolean {
    ...
    switch (this.lnavDataCdiScaleLabel.get()) {
      case Epic2FlightArea.Arrival:
      case Epic2FlightArea.Departure: {
  ...
  private monitorMessages(): void {
    ...
    MappedSubject.create(([withinOneMinuteTod, withinFiveSecTod, apMode, passingConstraintIn15Secs, selectedAltitude, constraintAltitude]) => {
      const oldVta = this.vtaActive;
      if (withinOneMinuteTod && !withinFiveSecTod && apMode === APVerticalModes.ALT) {
        this.vtaActive = true;
      } else if (passingConstraintIn15Secs && MathUtils.round(selectedAltitude, 100) !== MathUtils.round(constraintAltitude ?? 0, 100)) {
        this.vtaActive = true;
      } else {
        this.vtaActive = false;
      }

      if (this.vtaActive !== oldVta) {
        this.bus.getPublisher<Epic2FmsEvents>().pub('epic2_fms_vertical_track_alert', this.vtaActive, true);
      }
    },
      this.withinOneMinuteTod, this.withinFiveSecondsTod, this.apValues.verticalActive,
      this.passingConstraintIn15Secs, this.apValues.selectedAltitude, this.currentConstraintAltitude);

    // CHECK ALT CONSTRAINT message (sent if next constraint is violated by >150ft (~45m) in FLC/PATH)
    this.isAltConstraintViolated.sub((v) => {
      if (v) {
        this.fmsMessageTransmitter.sendMessage(FmsMessageKey.CheckAltConstraint);
      } else {
        this.fmsMessageTransmitter.clearMessage(FmsMessageKey.CheckAltConstraint);
      }
    });

    this.unableNextAlt.sub(v => {
      if (v) {
        this.fmsMessageTransmitter.sendMessage(FmsMessageKey.UnableNextAlt);
      } else {
        this.fmsMessageTransmitter.clearMessage(FmsMessageKey.UnableNextAlt);
      }
    });
  }
  ...
  /**
   * Manages the TOC BOC Details based on any set cruise altitude and FMS 3000 logic for display.
   * @param flightPlan The lateral flight plan
   * @param verticalPlan The vertical flight plan.
   * @param lateralLegIndex The current lateral leg index.
   * @param alongLegDistance The current along leg distance.
   * @param currentGroundSpeed The current ground speed
   * @param currentAltitudeMetric The current metric altitude.
   * @param cruiseAltitude The aircraft cruise altitude
   * @returns TocBocDetails.
   */
  private manageTocBocDetails(
    ...
    if (calculate) {
      ...
      // if (
      //   this.cruiseAltitude > 0
      //   // && tocBocDetails.distanceFromToc > this.todDisplayDistanceMeters
      //   // && verticalPlan.firstClimbConstraintLegIndex !== undefined
      //   // && tocBocDetails.tocConstraintIndex <= verticalPlan.
      // ) {

      //   tocBocDetails = VNavUtils.getTocBodDetails(verticalPlan, lateralLegIndex, alongLegDistance,
      //     UnitType.FOOT.convertTo(this.cruiseAltitude, UnitType.METER), 0, this.todBodDetails);
      // } else if (todBodDetails.distanceFromTod > this.todDisplayDistanceMeters) {
      //   todBodDetails.todLegIndex = -1;
      // }

      this.tocBocDetailsSub.set('bocLegIndex', tocBocDetails.bocLegIndex);
      this.tocBocDetailsSub.set('tocLegIndex', tocBocDetails.tocLegIndex);
      this.tocBocDetailsSub.set('tocLegDistance', tocBocDetails.tocLegDistance);
      this.tocBocDetailsSub.set('distanceFromBoc', tocBocDetails.distanceFromBoc);
      this.tocBocDetailsSub.set('distanceFromToc', tocBocDetails.distanceFromToc);
      this.tocBocDetailsSub.set('tocConstraintIndex', tocBocDetails.tocConstraintIndex);
      this.tocBocDetailsSub.set('tocAltitude', tocBocDetails.tocAltitude);

      this.tocCalculatedTimer = Date.now();

      return tocBocDetails;
    }
    return this.tocBocDetails;
  }
  ...
  /**
   * Publishes TOC/BOC details to simvars.
   * @param details The TOC/BOC details object.
   * @param key The key to publish.
   * @param value The value to publish.
   */
  private publishTOCBOCDetails(details: TocBocDetails, key: keyof TocBocDetails, value: number): void {
    switch (key) {
      case 'bocLegIndex':
  ...
  /**
   * Updates the VNAV flight phase
   * @returns void
   */
  private updateVnavFlightPhase(): void {
    const apVerticalMode = this.apValues.verticalActive.get();
    const isAltPreselectIgnored = apVerticalMode === APVerticalModes.GP || apVerticalMode === APVerticalModes.GS;
    const isApInAltMode = apVerticalMode === APVerticalModes.ALT || apVerticalMode === APVerticalModes.CAP;
    const altitude = MathUtils.round(this.currentAltitude, 100);
    if (
      (altitude >= this.cruiseAltitude && this.preselectedAltitude >= this.cruiseAltitude + 5000 && isAltPreselectIgnored !== true)
      || (altitude < this.cruiseAltitude && this.preselectedAltitude > altitude && isApInAltMode !== true && isAltPreselectIgnored !== true)
      || (this.originFacility && this.flightArea.get() === Epic2FlightArea.Departure
        && UnitType.NMILE.convertFrom(this.planePos.distance(this.originFacility.lat, this.originFacility.lon), UnitType.GA_RADIAN) < 4)
    ) {
      return this.vnavFlightPhase.set(Epic2VerticalFlightPhase.Climb);
    }

    const destElevation = this.destFacility && AirportUtils.getElevation(this.destFacility);
    const destDistance = this.destFacility && UnitType.NMILE.convertFrom(this.planePos.distance(this.destFacility.lat, this.destFacility.lon), UnitType.GA_RADIAN);
    if (
      (this.preselectedAltitude < altitude - 150)
      || (this.flightArea.get() === Epic2FlightArea.Arrival && destElevation && destDistance && destElevation - this.currentAltitude < 5000 && destDistance < 15)
      || (this.flightArea.get() === Epic2FlightArea.Approach)
    ) {
      return this.vnavFlightPhase.set(Epic2VerticalFlightPhase.Descent);
    }

    return this.vnavFlightPhase.set(Epic2VerticalFlightPhase.Cruise);
  }
}
