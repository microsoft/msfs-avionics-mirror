...
import { DcpEventPublisherType } from '../DCP/DcpEventPublisher';
import { DcpEvent } from '../DCP/DcpEvent';
...
export class PfdMenuViewService extends MenuViewService<PfdMenuViewKeys> {
  ...
  constructor(
    ...
    const dcpSubscriber = bus.getSubscriber<DcpEventPublisherType>();
    ...
    if (hEvent.startsWith('Generic_Upr')) {
      ...
      switch (evt) {
        ...
        case GuiHEvent.SOFTKEY_4L: {
          // ET
          this.bus.getPublisher<DcpEventPublisherType>().pub('dcpEvent', DcpEvent.DCP_ET);
          return false;
        }
        case GuiHEvent.SOFTKEY_1R: {
          ...
          this.bus.getPublisher<DcpEventPublisherType>().pub('dcpEvent', DcpEvent.DCP_FRMT);
          return true;
        }
        case GuiHEvent.SOFTKEY_2R: {
          ...
          // But this requires a larger refactor.
          // For now, we transmit the corresponding DCP event instead.
          this.bus.getPublisher<DcpEventPublisherType>().pub('dcpEvent', DcpEvent.DCP_TERR_WX);
          return true;
        }
        case GuiHEvent.SOFTKEY_3R: {
          ...
          // But this requires a larger refactor.
          // For now, we transmit the corresponding DCP event instead.
          this.bus.getPublisher<DcpEventPublisherType>().pub('dcpEvent', DcpEvent.DCP_TFC);
          return false;
        }
