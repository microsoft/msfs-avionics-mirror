...
import {
  ...
  Autopilot, ConsumerSubject, ControlEvents, DirectorState, EventBus, FlightPlanner, MappedSubject, MetricAltitudeSettingsManager, NavComSimVars, NavSourceType,
  ObjectSubject, RadioUtils, SimVarValueType, Subject, UnitType, VNavAltCaptureType
    ...
    switch (this.cdiSource.type) {
      ...
      case NavSourceType.Gps:
        if (this.apValues.approachIsActive.get()) {
          if (this.navToNavManagerToUse && this.navToNavManagerToUse.getArmableLateralMode() === APLateralModes.LOC) {
            return APLateralModes.LOC;
          } else {
