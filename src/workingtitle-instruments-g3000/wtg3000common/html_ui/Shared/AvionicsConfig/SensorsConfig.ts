...
export class SensorsConfig implements Config {
   ...
   */
  public constructor(baseInstrument: BaseInstrument, element: Element | undefined, factory: ConfigFactory) {
    if (element === undefined) {
      ...
      this.aoaDefinition = this.parseAoaDefinition(baseInstrument, element, factory);
   ...
   */
  private parseAoaDefinition(baseInstrument: BaseInstrument, element: Element, factory: ConfigFactory): AoaDefinition {
    ...
    const stallAoa = this.parseAoaDefinitionNumericFactory(aoaElement.querySelector(':scope>StallAoa'), factory);
    const zeroLiftAoa = this.parseAoaDefinitionNumericFactory(aoaElement.querySelector(':scope>ZeroLiftAoa'), factory);
   ...
   * `element` is `null`.
   */
  private parseAoaDefinitionNumericFactory(element: Element | null, factory: ConfigFactory): ((context: NumericBusConfigContext) => NumericConfigResult) | undefined {
    ...
    try {
      ...
      if (config === undefined || !('isNumericConfig' in config)) {
        console.warn(`Invalid SensorsConfig definition: angle of attack value ${element.tagName} is not a numeric config. Discarding option.`);
        return undefined;
      }
  ...
  private parseWeatherRadarDefinition(baseInstrument: BaseInstrument, element: Element): WeatherRadarDefinition | undefined {
    ...
    if (scanActiveCircuitElement) {
      ...
      const scanActiveCircuitProcedureIndexAttr = scanActiveCircuitElement.getAttribute('procedure');
      if (scanActiveCircuitProcedureIndexAttr !== null) {
        scanActiveCircuitProcedureIndex = Number(scanActiveCircuitProcedureIndexAttr);
