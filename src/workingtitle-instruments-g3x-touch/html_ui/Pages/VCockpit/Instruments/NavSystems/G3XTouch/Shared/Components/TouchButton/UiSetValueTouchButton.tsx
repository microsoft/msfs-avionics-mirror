import { DisplayComponent, FSComponent, MutableSubscribable, Subscribable, SubscribableSet, SubscribableType, ToggleableClassNameRecord, VNode } from '@microsoft/msfs-sdk';

import { SetValueTouchButtonProps, TouchButtonHoldAction, TouchButtonHoldEndReason, TouchButtonOnTouchedAction } from '@microsoft/msfs-garminsdk';
 ...
 */
export interface UiSetValueTouchButtonProps<S extends MutableSubscribable<any>>
  extends Omit<SetValueTouchButtonProps<S>, 'onTouched' | 'onPressed' | 'onHoldStarted' | 'onHoldTick' | 'onHoldEnded' | 'focusOnDrag' | 'class'> {
   ...
   */
  onHoldTick?: <B extends UiSetValueTouchButton<S> = UiSetValueTouchButton<S>>(
    button: B,
    state: S,
   ...
   */
  onHoldEnded?: <B extends UiSetValueTouchButton<S> = UiSetValueTouchButton<S>>(
    button: B,
    state: S,
    setValue: SubscribableType<S>,
 ...
 * focus.
 */
export class UiSetValueTouchButton<S extends MutableSubscribable<any>> extends DisplayComponent<UiSetValueTouchButtonProps<S>>
  implements UiFocusableComponent {

  /** @inheritdoc */
  public readonly isUiFocusableComponent = true;

  protected readonly buttonRef = FSComponent.createRef<SetValueTouchButton<S>>();
  ...
  protected onTouched(button: SetValueTouchButton<S>, state: S, setValue: SubscribableType<S>): TouchButtonOnTouchedAction {
    if (this.props.onTouched) {
      return this.props.onTouched(this, state, setValue);
    } else {
  ...
  protected onPressed(button: SetValueTouchButton<S>, state: S, setValue: SubscribableType<S>, isHeld: boolean): void {
    if (this.props.onPressed) {
      this.props.onPressed(this, state, setValue, isHeld);
    } else {
      state.set(setValue);
    }
  }
  ...
  protected onHoldStarted(button: SetValueTouchButton<S>, state: S, setValue: SubscribableType<S>): TouchButtonHoldAction {
    return this.props.onHoldStarted
      ? this.props.onHoldStarted(this, state, setValue)
   ...
   * @returns The action to take.
   */
  protected onHoldTick(
    button: SetValueTouchButton<S>,
    state: S,
    setValue: SubscribableType<S>,
    dt: number,
    totalTime: number,
    ...
    return this.props.onHoldTick
      ? this.props.onHoldTick(this, state, setValue, dt, totalTime, timeSinceLastPress)
      : TouchButtonHoldAction.None;
  }
  ...
  protected onHoldEnded(
    ...
    this.props.onHoldEnded && this.props.onHoldEnded(this, state, setValue, totalHoldDuration, endReason);
  }
