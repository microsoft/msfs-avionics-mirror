import {
  AirportFacility, AirwayData, ApproachTransition, EnrouteTransition, Facility, FacilitySearchType,
  FacilityType,
  GeoPoint, ICAO, IntersectionFacility,
...
export class DevPlanUtils {
   ...
   */
  public static async setOrigin(fms: Epic2Fms, ident: string): Promise<AirportFacility> {
   ...
   */
  public static setOriginRunway(fms: Epic2Fms, origin: AirportFacility, runwayName: string): OneWayRunway {
   ...
   */
  public static async setDestination(fms: Epic2Fms, ident: string): Promise<AirportFacility> {
   ...
   * @param fms The FMS.
   */
  public static removeOrigin(fms: Epic2Fms): void {
   ...
   * @param fms The FMS.
   */
  public static removeDestination(fms: Epic2Fms): void {
   ...
   * @returns The destination facility.
   */
  public static async insertWaypoint(fms: Epic2Fms, ident: string, segmentIndex: number, segmentLegIndex?: number): Promise<Facility> {
    const facility = await this.findNearestFacilityFromIdent(fms, ident);

    fms.insertWaypoint(facility, segmentIndex, segmentLegIndex);
   ...
   */
  public static async getAirwayFromLeg(fms: Epic2Fms, entryIdent: string, airwayName: string): Promise<AirwayData> {
   ...
   */
  public static async findNearestIntersectionFromIdent(fms: Epic2Fms, ident: string): Promise<IntersectionFacility> {
   ...
   * @returns The selected facility.
   */
  public static async findNearestFacilityFromIdent(fms: Epic2Fms, ident: string, facilityType = FacilitySearchType.All): Promise<Facility> {
    const ppos = fms.ppos;
   ...
   */
  public static loadDeparture(fms: Epic2Fms, origin: AirportFacility, departureName: string, runwayName: string, transitionName?: string): void {
   ...
   * @throws Error if something couldn't be found with given inputs.
   */
  public static loadArrival(fms: Epic2Fms, destination: AirportFacility, arrivalName: string, transitionName?: string, runwayTransitionName?: string): void {
   ...
   * @throws Error if something couldn't be found with given inputs.
   */
  public static async loadApproach(fms: Epic2Fms, destination: AirportFacility, approachName: string, transitionName?: string): Promise<void> {
    ...
    await fms.insertApproach({ facility: destination, approachIndex, approachTransitionIndex });
  }
   ...
   */
  // public static async directToRandom(fms: Epic2Fms, ident: string, course?: number): Promise<Facility> {
