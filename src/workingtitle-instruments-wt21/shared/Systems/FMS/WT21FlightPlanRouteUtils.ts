...
import { WT21LegDefinitionFlags } from './WT21FmsUtils';

/**
 * Methods for working with WT21 flight plan routes.
 */
export class WT21FlightPlanRouteUtils {
          ...
          segment.legs
            .filter(leg => {
              ...
              return FlightPlanUtils.isDiscontinuityLeg(leg.leg.type)
                || !BitFlags.isAny(leg.flags, LegDefinitionFlags.DirectTo | WT21LegDefinitionFlags.DisplacedActiveLeg);
            })
            .map(leg => WT21FlightPlanRouteUtils.legToFlightPlanRouteEnrouteLeg(facLoader, leg.leg))
        );
