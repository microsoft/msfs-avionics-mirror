...
/**
 * A manager of reference V-speed bugs. The manager will automatically hide takeoff V-speed bugs when gear or flaps are
 * extended from a fully retracted state (the latter only when in the air). The manager will also optionally hide
 * takeoff V-speed bugs when indicated airspeed exceeds a threshold value, if such a value is defined.
 */
export class VSpeedBugManager {
  ...
  public init(paused = false): void {
    ...
    if (this.takeoffVSpeedDefs.length === 0) {
      return;
    }
    ...
    const maxIas = (this.vSpeedSettingManager.vSpeedGroups.get(VSpeedGroupType.Takeoff) as TakeoffVSpeedGroup | undefined)?.maxIas;
    const maxAltitude = (this.vSpeedSettingManager.vSpeedGroups.get(VSpeedGroupType.Configuration) as ConfigurationVSpeedGroup | undefined)?.maxAltitude;
