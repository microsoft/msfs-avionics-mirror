...
import { DirectToState, LNavDataEvents, NavIndicatorController } from '@microsoft/msfs-garminsdk';
...
export class Fma extends DisplayComponent<FmaProps> {
  ...
  public onAfterRender(): void {
    ...
    ap.on('ap_master_status').handle(status => {
      this.apMaster.set(status && !this.fdNotInstalled);
    });
    ap.on('ap_yd_status').handle(this.ydActive.set.bind(this.ydActive));
  ...
  private getVerticalArmedString(v: APVerticalModes): string {
    if (this.autopilotModes.altitudeCaptureArmed || this.autopilotModes.verticalActive === APVerticalModes.PATH) {
      const alt = this.autopilotModes.verticalAltitudeArmed;
      return alt === APAltitudeModes.ALTS ? 'ALTS' : alt === APAltitudeModes.ALTV ? 'ALTV' : 'ALT';
    }
    switch (v) {
  ...
  private getVerticalApproachArmedString(v: APVerticalModes): string {
    switch (v) {
      case APVerticalModes.GP:
        if (this.autopilotModes.altitudeCaptureArmed && this.autopilotModes.verticalArmed === APVerticalModes.PATH) {
          return 'GP/V';
        }
        return 'GP';
      case APVerticalModes.GS:
  ...
  private handleVerticalValueChanged(): void {
    if (this.autopilotModes.verticalActive === APVerticalModes.VS) {
      ...
      this.verticalValueSubject.set(`${(Math.round(this.autopilotModes.altitudeCaptureValue))}`);
