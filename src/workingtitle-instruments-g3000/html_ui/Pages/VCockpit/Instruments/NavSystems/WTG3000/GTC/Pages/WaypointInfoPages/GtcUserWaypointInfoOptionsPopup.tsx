import { ArrayUtils, DisplayComponent, FacilityWaypoint, FSComponent, ICAO, MutableSubscribable, Subscribable, UserFacility, VNode } from '@microsoft/msfs-sdk';
...
export class GtcUserWaypointInfoOptionsPopup extends GtcView<GtcUserWaypointInfoOptionsPopupProps> {
  ...
  private async onDeleteButtonPressed(): Promise<void> {
    ...
    if (waypoint !== null) {
      ...
      const result = await GtcDialogs.openMessageDialog(this.props.gtcService, `Would you like to delete the user waypoint ${ICAO.getIdent(icao)}?`);

      if (result) {
        if (this.props.controller.getWaypointFlightPlanStatus(icao) !== UserWaypointFlightPlanStatus.None) {
          await GtcDialogs.openMessageDialog(this.props.gtcService, `The user waypoint ${ICAO.getIdent(icao)} is a flight plan waypoint and cannot be deleted.`, false);
        } else {
