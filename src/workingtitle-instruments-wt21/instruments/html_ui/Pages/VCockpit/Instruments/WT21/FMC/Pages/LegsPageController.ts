import {
  ...
  FmcListUtility, FmcRenderTemplate, FmcRenderTemplateRow, GeoPoint, GNSSEvents, ICAO, LegTurnDirection, LegType, LNavDataSimVarEvents, LNavEvents, MagVar,
  RawFormatter, SpeedRestrictionType, SpeedUnit, Subject, VNavConstraint, VNavLeg
...
export class LegsPageController {
  ...
  public renderRow = (
    ...
    prevData: LegPageItem | undefined,
    data: LegPageItem | undefined,
  ...
  private handlePlanActiveLegChangeLegs = (event: FlightPlanActiveLegEvent): void => {
    if (this.fms.getPlanIndexForFmcPage() === WT21FmsUtils.PRIMARY_MOD_PLAN_INDEX
  ...
  private handleFlightPlanCalculatedEventLegs = (event: FlightPlanCalculatedEvent): void => {
    if (event.planIndex === this.fms.getPlanIndexForFmcPage() && this.planChanged) {
      this.updateData();
    } else if (event.planIndex === WT21FmsUtils.PRIMARY_MOD_PLAN_INDEX) {
      if (this.fms.planInMod.get() && this.fms.getDirectToState(WT21FmsUtils.PRIMARY_MOD_PLAN_INDEX) === DirectToState.TOEXISTING) {
        this.fms.verticalPathCalculator.requestPathCompute(WT21FmsUtils.PRIMARY_MOD_PLAN_INDEX);
      }
    }
  };
  ...
  private handleVnavCalculatedLegs = (sender: any, planIndex: number): void => {
    const displayedPlanIndex = this.fms.planInMod.get() ? WT21FmsUtils.PRIMARY_MOD_PLAN_INDEX : WT21FmsUtils.PRIMARY_ACT_PLAN_INDEX;
  ...
  private updateData(): void {
    if (this.fms.hasFlightPlan(this.fms.getPlanIndexForFmcPage())) {
      ...
      const activeLegIndex = lateralPlan.planIndex === WT21FmsUtils.PRIMARY_ACT_PLAN_INDEX ? Math.max(lateralPlan.activeLateralLeg, this.effectiveLegIndex)
      ...
      if (scratchpadContents.startsWith('HOLD AT ')) {
        ...
        if (scratchpadRest === storeFacilityIdent) {
          ...
          if (holdInserted) {
            ...
            this.page.screen.navigateTo('/fpln-hold', { atLeg: addedLegGlobalIndex + 1 });
      ...
      if (this.isForHoldSelection) {
        ...
        try {
          ...
          if (atLeg !== null) {
            ...
            this.page.screen.navigateTo('/fpln-hold', { atLeg: atLeg - 1 });
  ...
  public isFmcPageInDirectToExistingState(): boolean {
    ...
    if (fmcPlan.planIndex === WT21FmsUtils.PRIMARY_ACT_PLAN_INDEX) {
