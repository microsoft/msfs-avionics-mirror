...
import { PfdTrimFlapGaugesConfig } from '../../PFD/Components/TrimFlapGauges/PfdTrimFlapGaugesConfig';
...
export class InstrumentConfig {
  ...
  public readonly pfdTrimFlapGauges: PfdTrimFlapGaugesConfig;
  ...
  constructor(
    ...
    this.pfdTrimFlapGauges = this.parsePfdTrimFlapGaugesConfig(g3xRoot, instrumentConfigElement);
  ...
  private parseHorizonConfig(config: Element | undefined, instrumentConfig: Element | undefined): HorizonConfig {
    ...
    if (config) {
      try {
        const horizon = config.querySelector(':scope>Horizon');
        if (horizon !== null) {
          return new HorizonConfig(horizon);
        }
      } catch (e) {
   ...
   */
  private parsePfdTrimFlapGaugesConfig(config: Element | undefined, instrumentConfig: Element | undefined): PfdTrimFlapGaugesConfig {
    if (instrumentConfig !== undefined) {
      try {
        const gauges = instrumentConfig.querySelector(':scope>PfdTrimFlapGauges');
        if (gauges !== null) {
          return new PfdTrimFlapGaugesConfig(gauges);
        }
      } catch (e) {
        console.warn(e);
      }
    ...
    if (config) {
      try {
        const gauges = config.querySelector(':scope>PfdTrimFlapGauges');
        if (gauges !== null) {
          return new PfdTrimFlapGaugesConfig(gauges);
        }
      } catch (e) {
    ...
    return new PfdTrimFlapGaugesConfig(undefined);
  }
