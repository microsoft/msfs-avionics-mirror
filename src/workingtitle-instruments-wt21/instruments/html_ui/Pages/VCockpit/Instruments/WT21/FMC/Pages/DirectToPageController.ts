import {
  AbstractFmcPage, AdcEvents, AltitudeRestrictionType, APEvents, ComputedSubject, ConsumerSubject, DisplayField, EventBus, FlightPlanActiveLegEvent,
  FlightPlanCalculatedEvent, FlightPlanCopiedEvent, FlightPlanLegEvent, FlightPlannerEvents, FmcListUtility, FmcRenderTemplate, FmcRenderTemplateRow, LegType,
  NavMath, RawFormatter, Subject, UnitType, VNavUtils
...
export class DirectToPageController {
  ...
  public legsList = new FmcListUtility<DirectToPageItem>(this.page, this.store.legs, this.renderRow, 4);
  ...
  constructor(
    ...
    private readonly screen: WT21FmcScreen,
  ) {
  }
  ...
  private lskL1Pressed = async (scratchpadContents: string, isDelete?: boolean): Promise<boolean | string> => {
    if (!isDelete && scratchpadContents) {
      const selectedFacility = await this.screen.selectWptFromIdent(scratchpadContents, this.fms.ppos);
  ...
  private lskR1Pressed = async (): Promise<boolean> => {
    this.screen.navigateTo('/nearest-airports');
    return true;
  };
