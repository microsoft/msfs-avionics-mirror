import {
  AdcEvents, AeroMath, AvionicsSystemState, AvionicsSystemStateEvent, ClockEvents, ConsumerSubject,
  ControlSurfacesEvents, EventBus, FacilityType, ICAO, MappedSubject, MappedSubscribable, Subject, Subscription,
  UnitType
...
import {
  ...
  ToldResetType, ToldRunwaySurfaceCondition, ToldTakeoffPerformanceResult, ToldThrustReverserSelectable,
  ToldUserSettings, VSpeedGroupType, VSpeedUserSettingManager, WeightFuelEvents
...
export class ToldComputer {
  ...
  private readonly zeroFuelWeight = ConsumerSubject.create<number>(null, 0);
  private readonly aircraftWeight = ConsumerSubject.create<number | null>(null, null);
  private readonly landingWeight = ConsumerSubject.create<number | null>(null, null);
  ...
  public constructor(
    ...
    private readonly toldManager: ToldManager,
  ...
  public init(module: ToldModule, paused = false): void {
    ...
    this.initDefaultTakeoffSettings();
    this.initDefaultLandingSettings();
    ...
    this.zeroFuelWeight.setConsumer(sub.on('weightfuel_zero_fuel_weight'));
    ...
    this.resetSub = this.toldManager.onReset.on(this.reset.bind(this));
   ...
   */
  private initDefaultTakeoffSettings(): void {
    ...
    this.toldSettingManager.getSetting('toldTakeoffFactor').resetToDefault();
   ...
   */
  private initDefaultLandingSettings(): void {
  ...
  private initWeightLogic(): void {
    ...
    this.landingWeightCanUsePredictedState = this.landingWeight.map(weight => weight !== null);
    ...
    this.landingWeightDestinationIcaoSub = this.toldManager.destinationAirport.sub(airport => {
      const flightPlanDestination = this.fms.hasPrimaryFlightPlan() ? this.fms.getPrimaryFlightPlan().destinationAirportIcao : undefined;

      if (
        this.landingWeightCanUsePredictedSetting.value
        && flightPlanDestination !== undefined
        && airport !== null
        && ICAO.valueEquals(flightPlanDestination, airport.icaoStruct)
      ) {
  ...
  private initTakeoffCalcListeners(): void {
    ...
    this.takeoffCalcAcceptedSubs.push(
      ...
      this.zeroFuelWeight.sub(requestTakeoffCalc, false, true)
    );
  }
  ...
  private initLandingCalcListeners(): void {
    ...
    this.landingCalcAcceptedSubs.push(
      this.zeroFuelWeight.sub(requestLandingCalc, false, true)
    );
  }
   ...
   */
  private reset(manager: ToldManager, type: ToldResetType): void {
  ...
  private loadEmergencyReturn(): void {
    ...
    if (!ICAO.isStringV2Facility(originIcao, FacilityType.RWY)) {
