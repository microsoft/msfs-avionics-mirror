import {
  BitFlags, ComponentProps, DigitScroller, DisplayComponent, FSComponent, MappedSubject, MappedSubscribable,
  MathUtils, MutableSubscribable, NodeReference, NumberFormatter, NumberUnitSubject, ObjectSubject, SetSubject,
  SubEvent, Subject, Subscribable, SubscribableMapFunctions, SubscribableSet, SubscribableUtils, Subscription,
  UnitType, UserSettingManager, VNode
} from '@microsoft/msfs-sdk';

import {
  AirspeedAlert, AirspeedDefinition, AirspeedIndicatorColorRange, AirspeedIndicatorColorRangeColor, AirspeedIndicatorColorRangeWidth,
  AirspeedIndicatorDataProvider, NumberUnitDisplay, VSpeedUserSettingTypes, VSpeedUserSettingUtils
...
import { AirspeedIndicatorGsDataProvider } from './AirspeedIndicatorGsDataProvider';

import './G3XAirspeedIndicator.css';

/**
 * Scale options for an airspeed tape.
 */
export type G3XAirspeedTapeScaleOptions = Pick<AirspeedTapeProps, 'minimum' | 'maximum' | 'window' | 'majorTickInterval' | 'minorTickFactor'>;

/**
 * Options for an airspeed trend vector.
 */
export type G3XAirspeedTrendVectorOptions = Pick<AirspeedTapeProps, 'trendThreshold'>;

/**
 * Options for airspeed alerts.
 */
export type G3XAirspeedAlertOptions = {
 ...
 */
export type G3XVSpeedBugDefinition = {
 ...
 */
export type G3XVSpeedBugOptions = {
 ...
 */
export interface G3XAirspeedIndicatorProps extends ComponentProps {
 ...
 */
export class G3XAirspeedIndicator extends DisplayComponent<G3XAirspeedIndicatorProps> {
  ...
  private readonly activeAlert = this.props.dataProvider.airspeedAlerts.map((alerts): AirspeedAlert => {
    ...
    if (BitFlags.isAny(alerts, AirspeedAlert.Underspeed)) {
      return AirspeedAlert.Underspeed;
    } else if (BitFlags.isAny(alerts, AirspeedAlert.Overspeed)) {
      return AirspeedAlert.Overspeed;
    } else if (BitFlags.isAny(alerts, AirspeedAlert.TrendUnderspeed)) {
      return AirspeedAlert.TrendUnderspeed;
    } else if (BitFlags.isAny(alerts, AirspeedAlert.TrendOverspeed)) {
  ...
  private readonly tapeMinimum = SubscribableUtils.toSubscribable(this.props.tapeScaleOptions.minimum, true);
  private readonly tapeMaximum = SubscribableUtils.toSubscribable(this.props.tapeScaleOptions.maximum, true);

  private readonly isIasOffscale = MappedSubject.create(
    ([ias, min, max]) => ias < min || ias > max,
  ...
  private readonly isTasDisplayVisible = Subject.create(false);
  private readonly isTopDisplayVisible = MappedSubject.create(
    SubscribableMapFunctions.or(),
    this.isReferenceDisplayVisible,
    this.isTasDisplayVisible
  );
  ...
  /** @inheritdoc */
  public render(): VNode {
    const showTas = MappedSubject.create(
      ([showTop, isReferenceVisible]) => showTop && !isReferenceVisible,
      this.showTopBottomDisplays,
      this.isReferenceDisplayVisible
    );
    ...
    return (
      <div class={this.rootCssClass} data-checklist="checklist-airspeed">
        <AirspeedTape
          ...
          minimum={this.tapeMinimum}
          maximum={this.tapeMaximum}
          window={this.props.tapeScaleOptions.window}
          majorTickInterval={this.props.tapeScaleOptions.majorTickInterval}
          minorTickFactor={this.props.tapeScaleOptions.minorTickFactor}
        ...
        <G3XFailureBox
          ...
          label={
            <>
              <div class='airspeed-tape-failure-box-label-word-1'>A<br />I<br />R<br />S<br />P<br />E<br />E<br />D</div>
              <div class='airspeed-tape-failure-box-label-word-2'>F<br />A<br />I<br />L</div>
            </>
          }
          class='airspeed-tape-failure-box'
        />
        ...
        <div class='airspeed-top-container' data-checklist="checklist-airspeed-top">
          <AirspeedTasDisplay
            ...
            isOffScale={this.isIasOffscale}
            isDataFailed={this.props.dataProvider.isDataFailed}
            isVisible={this.isTasDisplayVisible}
          />
        ...
        <div class='airspeed-bottom-container' data-checklist="checklist-airspeed-bottom">
          <AirspeedGroundSpeedDisplay
            show={this.showTopBottomDisplays}
            gsKnots={this.props.gsDataProvider.gsKnots}
            isDataFailed={this.props.gsDataProvider.isDataFailed}
            isVisible={this.isBottomDisplayVisible}
          />
        </div>
      </div>
    );
...
class AirspeedTape extends DisplayComponent<AirspeedTapeProps> {
  ...
  private readonly iasTapeValue = MappedSubject.create(
    ([iasKnots, minimum, maximum, isDataFailed]): number => {
      return isDataFailed ? minimum : MathUtils.clamp(iasKnots, minimum, maximum);
    },
    this.props.dataProvider.iasKnots,
    this.minimum,
    this.maximum,
  ...
  private readonly colorRangeDefs: AirspeedDefinition[] = [];
  ...
  /** @inheritdoc */
  public render(): VNode {
    const { bugs: vSpeedBugs, offscale: vSpeedOffscaleLabels } = this.renderVSpeedBugs();
    return (
  ...
  private renderColorRanges(): VNode[] {
    ...
    for (const definition of this.props.colorRanges) {
      ...
      this.colorRangeDefs.push(minimum);
      this.colorRangeDefs.push(maximum);
  ...
  public destroy(): void {
    ...
    for (const def of this.colorRangeDefs) {
      def.destroy?.();
    }
...
/**
 * An indicated airspeed display box for a G3X Touch airspeed tape.
 */
      ...
      <div class='airspeed-ias-box' style={this.rootStyle}>
        <svg viewBox="0 0 82 72" class='airspeed-ias-box-bg' preserveAspectRatio='none'>
          <path
            ...
            d='M 75 43 v 25 c 0 2.2 -1.8 4 -4 4 h -13 c -2.2 0 -4 -1.8 -4 -4 v -14 h -50 c -2.2 0 -4 -1.8 -4 -4 v -28 c 0 -2.2 1.8 -4 4 -4 h 50 v -14 c 0 -2.2 1.8 -4 4 -4 h 13 c 2.2 0 4 1.8 4 4 v 25 l 7 7 l -7 7 Z'
          />
        </svg>
...
/**
 * A speed bug for a G3X Touch airspeed tape.
 */
...
class ReferenceSpeedBug extends DisplayComponent<ReferenceSpeedBugProps> {
  ...
  public render(): VNode {
    return (
      ...
      >
        <svg viewBox='0 0 15 36' preserveAspectRatio='none' class='airspeed-refspeed-bug-icon airspeed-refspeed-bug-icon-manual'>
          <path d="M 0 0 h 15 v 36 h -15 v -11 l 7 -7 l -7 -7 Z" vector-effect='non-scaling-stroke' />
        </svg>
      </SpeedBug>
    );
  }
  ...
  public render(): VNode {
    return (
      <div class='airspeed-refspeed-container' style={this.rootStyle}>
        <svg class='airspeed-refspeed-icon airspeed-refspeed-icon-manual' viewBox='0 0 90 90' preserveAspectRatio='none'>
          <path d='M 0 0 h 90 v 90 h -90 v -30 l 50 -15 l -50 -20 Z' vector-effect='non-scaling-stroke' />
        </svg>
...
interface AirspeedTasDisplayProps extends ComponentProps {
  ...
  /** Whether airspeed is currently off-scale. */
...
class AirspeedTasDisplay extends DisplayComponent<AirspeedTasDisplayProps> {
  ...
  private isOffScaleSub?: Subscription;
  private isDataFailedSub?: Subscription;
  ...
  public onAfterRender(): void {
    ...
    const isOffScaleSub = this.isOffScaleSub = this.props.isOffScale.sub(isOffScale => {
      if (isOffScale) {
        tasSub.pause();
        this.tas.set(NaN);
      } else {
  ...
  public render(): VNode {
    return (
      <div class='airspeed-tas-display' style={{ 'display': this.rootDisplay }}>
        <div class='airspeed-speed-display airspeed-tas-display-main' style={{ 'display': this.mainDisplay }}>
          <div class='airspeed-speed-display-title'>TAS</div>
          <G3XNumberUnitDisplay
            value={this.tas}
            displayUnit={UnitType.KNOT}
            formatter={NumberFormatter.create({ precision: 1, nanString: '___' })}
            useBasicUnitFormat
            class='airspeed-speed-display-value'
          />
        </div>
        ...
        <G3XFailureBox
          ...
          label='TAS'
          class='airspeed-tas-failure-box'
        />
      </div>
    );
  }
...
/**
 * Component props for AirspeedGroundSpeedDisplay.
 */
interface AirspeedGroundSpeedDisplayProps extends ComponentProps {
  /** Whether to the show the display. */
  show: Subscribable<boolean>;

  /** The current ground speed, in knots. */
  gsKnots: Subscribable<number>;
...
/**
 * A ground speed display for an airspeed indicator.
 */
class AirspeedGroundSpeedDisplay extends DisplayComponent<AirspeedGroundSpeedDisplayProps> {
  private readonly rootDisplay = Subject.create('none');

  private readonly groundSpeed = NumberUnitSubject.create(UnitType.KNOT.createNumber(0));

  private showSub?: Subscription;
  private groundSpeedSub?: Subscription;
  private isDataFailedSub?: Subscription;

  /** @inheritdoc */
  public onAfterRender(): void {
    const gsSub = this.groundSpeedSub = this.props.gsKnots.sub(this.update.bind(this), false, true);

    const isDataFailedSub = this.isDataFailedSub = this.props.isDataFailed.sub(isFailed => {
      if (isFailed) {
        gsSub.pause();
        this.groundSpeed.set(NaN);
      } else {
        gsSub.resume(true);
      }
    }, false, true);
  ...
  /**
   * Updates this display.
   * @param gsKnots The current ground speed, in knots.
   */
  private update(gsKnots: number): void {
    this.groundSpeed.set(MathUtils.round(gsKnots, 1));
  }

  /** @inheritdoc */
  public render(): VNode {
    return (
      <div class='airspeed-speed-display airspeed-gs-display' style={{ 'display': this.rootDisplay }}>
        <div class='airspeed-speed-display-title'>GS</div>
        <G3XNumberUnitDisplay
