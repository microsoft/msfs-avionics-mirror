import {
  APAltitudeModes, APEvents, APLateralModes, APVerticalModes, ConsumerSubject, DisplayComponent, EventBus, FlightPlanner, FSComponent, ObjectSubject,
  SetSubject, Subject, Subscription, VNavEvents, VNavPathMode, VNode
...
export class Fma extends DisplayComponent<FmaProps> {
  ...
  private updateFromFmaData(data: FmaData, vnavPathMode: VNavPathMode, lastData?: FmaData, lastVNavPathMode?: VNavPathMode): void {
    ...
    if (data.verticalActive !== lastData?.verticalActive) {
      ...
      this.altitudeCaptureValue.set(data.altitideCaptureValue);
      this.verticalSlotActiveData.set(this.tempSlotActiveData);
    }
   ...
   */
  private getVerticalActiveString(mode: APVerticalModes, altCapMode: APAltitudeModes): string {
    switch (mode) {
      case APVerticalModes.VS:
        return 'VS';
      case APVerticalModes.FLC:
   ...
   * capture mode, and VNAV path mode.
   */
  private getVerticalArmedString(mode: APVerticalModes, activeMode: APVerticalModes, isAltCapArmed: boolean, altCapMode: APAltitudeModes, vnavPathMode: VNavPathMode): string {
   ...
   * altitude capture mode arm state.
   */
  private getVerticalApproachArmedString(mode: APVerticalModes, verticalArmedMode: APVerticalModes, isAltCapArmed: boolean): string {
    switch (mode) {
      case APVerticalModes.GP:
   ...
   */
  private getLateralActiveString(mode: APLateralModes): string {
    switch (mode) {
   ...
   */
  private getLateralArmedString(mode: APLateralModes): string {
    switch (mode) {
      case APLateralModes.HEADING:
        return 'HDG';
      case APLateralModes.LOC:
