...
import { ControllableDisplayPaneIndex, TakeoffPerfConfigurationOptions, ToldThrustReverserSelectable, ToldUserSettingTypes } from '@microsoft/msfs-wtg3000-common';
...
import { GtcToldConfigDefaultsPopup, GtcToldConfigDefaultsPopupEnumOption } from './GtcToldConfigDefaultsPopup';
 ...
 */
export interface GtcTakeoffDataPageConfigTabProps extends ComponentProps {
  /** The GTC service. */
  ...
  options: Readonly<TakeoffPerfConfigurationOptions>;
...
/**
 * GTC view keys for popups owned by takeoff data page config tabs.
 */
enum GtcTakeoffDataPageConfigTabPopupKeys {
  TakeoffConfigDefaults = 'TakeoffConfigDefaults'
}

/**
 * A GTC takeoff data page config tab.
 */
export class GtcTakeoffDataPageConfigTab extends DisplayComponent<GtcTakeoffDataPageConfigTabProps> implements GtcToldDataPageTabContent {
  ...
  private readonly rollingOptions = this.props.options.rolling;

  private readonly isDefaultsButtonEnabled = (this.flapsOptions !== undefined && this.flapsOptions.options.length > 1) || this.rollingOptions !== undefined;

  private readonly runwaySurfaceSetting = this.props.toldSettingManager.getSetting('toldTakeoffRunwaySurface');
  private readonly flapsIndexSetting = this.props.toldSettingManager.getSetting('toldTakeoffFlapsIndex');
  private readonly antiIceOnSetting = this.props.toldSettingManager.getSetting('toldTakeoffAntiIceOn');
  private readonly factorSetting = this.props.toldSettingManager.getSetting('toldTakeoffFactor');

  private readonly isAntiIceAllowed = this.antiIceOptions === undefined
    ? undefined
    : this.props.toldSettingManager.getSetting('toldTakeoffTemperature').map(temp => temp > Number.MIN_SAFE_INTEGER && temp <= this.antiIceOptions!.maxTemp.asUnit(UnitType.CELSIUS));
  ...
  public onAfterRender(thisNode: VNode): void {
    ...
    this.props.gtcService.registerView(
      ...
      GtcTakeoffDataPageConfigTabPopupKeys.TakeoffConfigDefaults,
  ...
  private resetToDefaults(): void {
    this.factorSetting.value = 100;

    if (this.flapsOptions !== undefined) {
      const defaultVal = Math.max(this.props.toldSettingManager.getSetting('toldTakeoffFlapsIndexDefault').value, 0);
    ...
    if (this.thrustReverserOptions !== undefined) {
      this.props.toldSettingManager.getSetting('toldTakeoffThrustReversers').value = false;
    }
    if (this.rollingOptions !== undefined) {
      const defaultVal = this.props.toldSettingManager.getSetting('toldTakeoffRollingDefault').value === 1;
      this.props.toldSettingManager.getSetting('toldTakeoffRolling').value = defaultVal;
    }
  }
  ...
  private changeDefaults(): void {
    this.props.gtcService.openPopup(GtcTakeoffDataPageConfigTabPopupKeys.TakeoffConfigDefaults);
  }
  ...
  public render(): VNode {
    return (
      <div class='told-data-page-config takeoff-data-page-config'>
        <div class='told-data-page-config-left'>
          ...
          <GtcTouchButton
            ...
            isEnabled={this.isDefaultsButtonEnabled}
            class='told-data-page-config-left-button'
          />
        </div>
            ...
            {this.flapsOptions !== undefined && (
              <GtcListItem hideBorder>
                <GtcListSelectTouchButton
                  ...
                  class='told-data-page-config-right-button takeoff-data-page-config-flaps-button'
                />
              </GtcListItem>
            )}
            {this.antiIceOptions !== undefined && (
              <GtcListItem hideBorder>
                <GtcListSelectTouchButton
                  ...
                  class='told-data-page-config-right-button takeoff-data-page-config-antiice-button'
                />
              </GtcListItem>
            )}
            {this.thrustReverserOptions !== undefined && (
              <GtcListItem hideBorder>
                <GtcListSelectTouchButton
                  ...
                  state={this.props.toldSettingManager.getSetting('toldTakeoffThrustReversers')}
                  ...
                  class='told-data-page-config-right-button takeoff-data-page-config-tr-button'
                />
              </GtcListItem>
            )}
            <GtcListItem hideBorder>
              <GtcValueTouchButton
                ...
                renderValue={GtcTakeoffDataPageConfigTab.FACTOR_FORMATTER}
                onPressed={this.selectFactor.bind(this)}
                class='told-data-page-config-right-button takeoff-data-page-config-factor-button'
              />
            </GtcListItem>
            {this.rollingOptions !== undefined && (
              <GtcListItem hideBorder>
                <GtcListSelectTouchButton
                  gtcService={this.props.gtcService}
                  listDialogKey={GtcViewKeys.ListDialog1}
                  label='Rolling Takeoff'
                  state={this.props.toldSettingManager.getSetting('toldTakeoffRolling')}
                  renderValue={value => value ? 'Yes' : 'No'}
                  listParams={state => {
                    return {
                      title: 'Rolling Takeoff',
                      inputData: [
                        {
                          value: false,
                          labelRenderer: () => 'No'
                  ...
                  class='told-data-page-config-right-button takeoff-data-page-config-rolling-button'
                />
              </GtcListItem>
            )}
          </GtcList>
  ...
  private renderOptionDefaultsPopup(gtcService: GtcService, controlMode: GtcControlMode, displayPaneIndex?: ControllableDisplayPaneIndex): VNode {
    const options: GtcToldConfigDefaultsPopupEnumOption<any>[] = [];

    if (this.flapsOptions !== undefined && this.flapsOptions.options.length > 1) {
      options.push({
        name: 'Takeoff Flaps',
        setting: this.props.toldSettingManager.getSetting('toldTakeoffFlapsIndexDefault'),
    ...
    if (this.rollingOptions !== undefined) {
      options.push({
        name: 'Rolling Takeoff',
        setting: this.props.toldSettingManager.getSetting('toldTakeoffRollingDefault'),
        values: [0, 1],
        renderValue: value => value === 1 ? 'Yes' : 'No'
      });
    }
