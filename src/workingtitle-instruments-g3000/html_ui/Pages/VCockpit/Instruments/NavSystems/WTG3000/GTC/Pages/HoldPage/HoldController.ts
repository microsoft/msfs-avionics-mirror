import {
  BitFlags, FacilityUtils, FixTypeFlags, FlightPlan, FlightPlanLeg, FlightPlanUtils, LegTurnDirection, LegType, NavMath,
  UnitType
...
export class HoldController {
   ...
   */
  public static createAndInsertHold(fms: Fms, planIndex: number, holdInfo: HoldInfo, fixIcao: string, segmentIndex: number, segmentLegIndex: number): boolean {
    ...
    if (nextLeg && FlightPlanUtils.isHoldLeg(nextLeg.leg.type) && nextLeg.leg.fixIcao === fixIcao) {
    ...
    Object.assign(leg, {
      turnDirection: holdInfo.turnDirection,
      course: holdInfo.holdCourseDirection === HoldCourseDirection.Outbound
        ? NavMath.normalizeHeading(holdInfo.course.number + 180)
        : holdInfo.course.number,
      distance: legMode === HoldLegMode.Time
        ? holdInfo.legTime.asUnit(UnitType.MINUTE)
        : holdInfo.legDistance.asUnit(UnitType.METER),
      distanceMinutes: legMode === HoldLegMode.Time,
      fixIcao: fixIcao,
    });
