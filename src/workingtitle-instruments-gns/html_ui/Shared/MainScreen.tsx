import {
  ...
  DisplayComponent, EventBus, FacilityLoader, FacilityRepository, FlightPathAirplaneSpeedMode, FlightPathAirplaneWindMode, FlightPathCalculator,
  FlightPlanner, FlightPlanRouteManager, FSComponent, GameStateProvider, GNSSPublisher, GPSSatComputer, GPSSatComputerEvents, GpsSynchronizer,
  GPSSystemState, HEvent, InstrumentBackplane, IntersectionType, LNavComputer, LNavControlEvents, LNavObsManager,
  LNavObsSimVarPublisher, LNavUtils, NavRadioIndex, NearestContext, RadioType, SBASGroupName,
  SetSubject, SimVarValueType, TrafficInstrument, VNavDataEventPublisher, VNavSimVarPublisher, VNode, Wait
} from '@microsoft/msfs-sdk';

import {
  DefaultObsSuspDataProvider, FmaData, Fms, GarminAdsb, GarminAPUtils, GarminFlightPlanRouteSyncManager,
  GarminGlidepathComputer, GarminNavToNavComputer, GarminObsLNavModule, GarminPrimaryFlightPlanRouteLoader,
  GarminPrimaryFlightPlanRouteProvider, NavdataComputer, NavEventsPublisher, ObsSuspModes, TrafficAdvisorySystem
...
export class MainScreen extends DisplayComponent<MainScreenProps> {
  ...
  public readonly calculator: FlightPathCalculator;
  public readonly fms: Fms;

  private readonly flightPlanRouteSyncManager?: GarminFlightPlanRouteSyncManager;
  ...
  constructor(props: MainScreenProps) {
    ...
    this.gpsSatComputer = new GPSSatComputer(
      1,
      this.props.bus,
      `${GNSFilePaths.ASSETS_PATH}/Data/gps_ephemeris.json`,
      `${GNSFilePaths.ASSETS_PATH}/Data/gps_sbas.json`,
      5000,
  ...
  private init(): void {
    ...
    NearestContext.onInitialized(() => {
      ...
      NearestContext.getInstance().usrs.setFacilityFilter(facility => facility.icaoStruct.airport === GnsFacilityUtils.USER_FACILITY_SCOPE);
  ...
  private async initFlightPlanAndAp(): Promise<void> {
    ...
    if (this.isFmsPrimary) {
      this.initFlightPlanRouteSync(manager!);
  ...
  private async initNavigationLoop(): Promise<void> {
    if (this.isPrimaryInstrument && !this.props.options.disableAutopilot) {
      this.initAutopilot();
    }
