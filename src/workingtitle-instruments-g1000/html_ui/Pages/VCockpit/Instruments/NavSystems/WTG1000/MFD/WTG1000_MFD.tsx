...
import {
  AdcPublisher, APRadioNavInstrument, AutopilotInstrument, BaseInstrumentPublisher, BottomTargetPathCalculator, Clock, CompositeLogicXMLHost, ControlPublisher,
  EISPublisher, ElectricalPublisher, EventBus, EventSubscriber, FacilityLoader, FacilityRepository, FlightPathCalculator, FlightPlanner, FSComponent,
  GNSSPublisher, GpsSynchronizer, HEvent, HEventPublisher, InstrumentBackplane, InstrumentEvents, LNavSimVarPublisher, MinimumsManager, MinimumsSimVarPublisher,
  NavComSimVarPublisher, PluginSystem, SimVarValueType, TrafficInstrument, UserSettingSaveManager, VNavSimVarPublisher, XMLGaugeConfigFactory
} from 'msfssdk';

import {
  DateTimeUserSettings, Fms, GarminAdsb, GarminAPConfig, GarminAPStateManager, LNavDataSimVarPublisher, NavdataComputer, TrafficAdvisorySystem,
  UnitsUserSettings
} from 'garminsdk';
...
class WTG1000_MFD extends BaseInstrument {
  ...
  constructor() {
    ...
    this.adc = new AdcPublisher(this.bus, 1, 1);
    ...
    this.calculator = new FlightPathCalculator(this.loader, { defaultClimbRate: 300, defaultSpeed: 85, bankAngle: 17.5 }, this.bus);
  ...
  public connectedCallback(): void {
    ...
    this.pluginSystem.startSystem({ bus: this.bus, viewService: this.viewService, menuSystem: menuSystem }).then(() => {
      ...
      menuSystem.addMenu('wxr-root', new MFDWeatherRadarRootMenu(menuSystem));
      menuSystem.addMenu('wxr-mode', new MFDWeatherRadarModeMenu(menuSystem, this.viewService));
      ...
      if (this.airframeOptions.hasWeatherRadar) {
        this.viewService.registerView('WeatherRadarPage', () => <MFDWeatherRadarPage viewService={this.viewService} bus={this.bus} menuSystem={menuSystem} />);
      }

      this.viewService.registerView('SelectProcedurePage', () => <MFDSelectProcedurePage viewService={this.viewService} fms={this.fms} bus={this.bus} calculator={this.calculator} menuSystem={menuSystem} hasRadioAltimeter={this.airframeOptions.hasRadioAltimeter} />);
      ...
      this.viewService.registerView('AirportInformation', () => <MFDAirportInformationPage viewService={this.viewService} fms={this.fms} menuSystem={menuSystem} bus={this.bus} facilityLoader={this.loader} controlPublisher={this.controlPublisher} />);
      this.viewService.registerView('IntersectionInformation', () => <MFDIntersectionInformationPage viewService={this.viewService} fms={this.fms} menuSystem={menuSystem} bus={this.bus} facilityLoader={this.loader} controlPublisher={this.controlPublisher} />);
      this.viewService.registerView('NdbInformation', () => <MFDNdbInformationPage viewService={this.viewService} fms={this.fms} menuSystem={menuSystem} bus={this.bus} facilityLoader={this.loader} controlPublisher={this.controlPublisher} />);
      this.viewService.registerView('VorInformation', () => <MFDVorInformationPage viewService={this.viewService} fms={this.fms} menuSystem={menuSystem} bus={this.bus} facilityLoader={this.loader} controlPublisher={this.controlPublisher} />);
      ...
      this.viewService.registerView('PageSelect', () => <MFDPageSelect viewService={this.viewService} title="Page Select" showTitle={false} supportWeatherRadarPage={this.airframeOptions.hasWeatherRadar} />);
