...
export class FmsSpeedManager {
  ...
  private readonly airframeMaxIas = SubscribableUtils.toSubscribable(this.config.airframeLimits.ias.resolve()(this.airframeContext).value, true)
    .map(SubscribableMapFunctions.withPrecision(1));
  private readonly airframeMaxMach = SubscribableUtils.toSubscribable(this.config.airframeLimits.mach.resolve()(this.airframeContext).value, true)
    ...
    if (!this.isOnGround.get()) {
      ...
      switch (fmaData?.verticalActive) {
        ...
        case APVerticalModes.CAP:
          ...
          return fmaData.altitudeCaptureValue < altitudeCeiling ? FmsAltitudeLimitState.InEffect : FmsAltitudeLimitState.AnticipationNotNeeded;
        default:
          anticipationNeeded = false;
      }
    } else {
