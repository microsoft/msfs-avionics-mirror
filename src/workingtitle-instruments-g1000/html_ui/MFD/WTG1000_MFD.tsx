...
import {
  ...
  FlightPlanRouteManager, FSComponent, GameStateProvider, GNSSPublisher, GpsSynchronizer, HEvent, HEventPublisher,
  InstrumentBackplane, InstrumentEvents, LNavComputer, LNavObsManager, LNavObsSimVarPublisher,
  MinimumsManager, MinimumsSimVarPublisher, NavComSimVarPublisher, NavEvents, NavSourceType, PluginSystem,
  SimVarValueType, SmoothingPathCalculator, TrafficInstrument, UserSettingSaveManager, Value, VNavSimVarPublisher, VNode,
  Wait, XMLGaugeConfigFactory
...
class WTG1000_MFD extends BaseInstrument {
  ...
  private navToNavComputer!: GarminNavToNavComputer;
  ...
  private flightPlanRouteSyncManager!: GarminFlightPlanRouteSyncManager;
  ...
  public connectedCallback(): void {
    ...
    const lnavMaxBankAngle = (): number => {
      return this.autopilot.apValues.maxBankId.get() === 1
        ? Math.min(this.airframeOptions.autopilotConfig.lnavOptions.maxBankAngle, this.airframeOptions.autopilotConfig.lowBankOptions.maxBankAngle)
        : this.airframeOptions.autopilotConfig.lnavOptions.maxBankAngle;
    };
    this.lnavComputer = new LNavComputer(
      0,
      this.bus,
      this.planner,
      new GarminObsLNavModule(0, this.bus, this.planner, {
    ...
    this.navToNavComputer = new GarminNavToNavComputer(this.bus, this.fms, {
      navRadioIndexes: [1, 2]
    });
    ...
    const apConfig = new GarminAPConfig(this.bus, {
      ...
      internalVNavComputer: apValues => {
        return new GarminVNavComputer(0, this.bus, this.fms.flightPlanner, this.verticalPathCalculator, apValues);
      },
      internalGlidepathComputer: () => {
        return new GarminGlidepathComputer(0, this.bus, this.fms.flightPlanner, {
    ...
    this.flightPlanRouteSyncManager = new GarminFlightPlanRouteSyncManager();
  ...
  private async initNavigationLoop(): Promise<void> {
    ...
    const busRealTime = ConsumerValue.create(sub.on('realTime'), 0);

    sub.on('simTimeHiFreq').handle(() => {
      const realTime = Date.now();
      // If the timestamp from the bus diverges from the actual timestamp by more than 2 seconds, then that usually
      // means the sim is paused in a menu.
      const isPaused = Math.abs(realTime - busRealTime.get()) > 2000;
