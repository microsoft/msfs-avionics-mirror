import { BitFlags, EventBus, FSComponent, Subject, UserSettingManager, VNode } from '@microsoft/msfs-sdk';

import { CheckBox, GuiDialog, GuiDialogProps, MapSettingsMfdAliased, MapWaypointsDisplay, PopupSubMenu } from '@microsoft/msfs-wt21-shared';
...
export class MfdLwrMapSymbolsMenu extends GuiDialog<MfdLwrMapSymbolsMenuProps> {
  ...
  private readonly mapSymbolsSetting = this.props.mapSettingsManager.getSetting('mapWaypointsDisplay');
  ...
  public onAfterRender(node: VNode): void {
    ...
    this.props.mapSettingsManager.whenSettingChanged('mapWaypointsDisplay').handle(x => {
  ...
  private linkRngAltSelApprOption(): void {
    this.rngAltSelOption.sub(v => {
      this.props.mapSettingsManager.getSetting('mapAltitudeArcShow').set(!!v);
    });
    this.props.mapSettingsManager.whenSettingChanged('mapAltitudeArcShow').handle(x => {
