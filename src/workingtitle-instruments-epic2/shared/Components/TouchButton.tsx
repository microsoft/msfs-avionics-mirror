import {
  ComponentProps, DisplayComponent, FSComponent, ObjectSubject, SetSubject, StyleRecord, Subject,
  Subscribable, SubscribableMapFunctions, SubscribableSet, SubscribableUtils, Subscription, VNode
} from '@microsoft/msfs-sdk';

import './BarTouchButtonType.css';
import './TabTouchButton.css';
import './TouchButton.css';
import './TouchButtonListButton.css';
...
export interface TouchButtonProps extends ComponentProps {
   ...
   * When true, the `touch-button-active` css class will be applied, giving it a cyan border and dark grey background.
   * Defaults to `false`.
   */
  isActive?: boolean | Subscribable<boolean>;
  ...
  variant: 'bar' | 'bar-lime' | 'base' | 'small' | 'bar-tab' | 'bar-menu' | 'list-button';
}
...
export class TouchButton<P extends TouchButtonProps = TouchButtonProps> extends DisplayComponent<P> {
  protected static readonly RESERVED_CSS_CLASSES = new Set([
    ...
    'touch-button-hidden',
    'touch-button-bar',
    'touch-button-bar-menu',
    'touch-button-bar-lime',
    'touch-button-small',
    'touch-button-tab',
    'touch-button-list-button',
  ]);

  protected action = Subject.create<TouchButtonOnTouchedAction>(TouchButtonOnTouchedAction.Ignore);
  protected styles = ObjectSubject.create<StyleRecord>({});

  protected readonly rootRef = FSComponent.createRef<HTMLDivElement>();
  protected readonly actualRef = this.props.ref ? this.props.ref : this.rootRef;
  ...
  protected readonly mouseEnterListener = this.onMouseEnter.bind(this);
  ...
  protected readonly isActive = SubscribableUtils.toSubscribable(this.props.isActive ?? false, true);
  ...
  protected isActiveSub?: Subscription;
  ...
  protected readonly inhibitOnDrag = this.props.inhibitOnDrag ?? this.props.isInList ?? false;
  protected readonly dragThresholdPxActual = this.props.dragThresholdPx ?? 40;
  protected readonly inhibitOnDragAxisActual = this.props.inhibitOnDragAxis ?? (this.props.isInList ? (this.props.listScrollAxis ?? 'y') : 'both');
  protected readonly variant = this.props.variant ?? '';
  ...
  public onAfterRender(): void {
    ...
    this.isActiveSub = this.isActive.sub(isActive => {
      if (isActive) {
        this.cssClassSet.add('touch-button-active');
      } else {
        this.cssClassSet.delete('touch-button-active');
      }
    }, true);
    ...
    switch (this.variant) {
      case 'base':
        this.cssClassSet.add('touch-button-base');
        break;
      case 'bar':
        this.cssClassSet.add('touch-button-base');
        this.cssClassSet.add('touch-button-bar');
        break;
      case 'bar-menu':
        this.cssClassSet.add('touch-button-base');
        this.cssClassSet.add('touch-button-bar');
        this.cssClassSet.add('touch-button-bar-menu');
        break;
      case 'bar-lime':
        this.cssClassSet.add('touch-button-base');
        this.cssClassSet.add('touch-button-bar');
        this.cssClassSet.add('touch-button-bar-lime');
        break;
      case 'small':
        this.cssClassSet.add('touch-button-base');
        this.cssClassSet.add('touch-button-small');
        break;
      case 'bar-tab':
        this.cssClassSet.add('touch-button-base');
        this.cssClassSet.add('touch-button-tab');
        break;
      case 'list-button':
        this.cssClassSet.add('touch-button-list-button');
        break;
    }

    this.actualRef.instance.addEventListener('mousedown', this.mouseDownListener);
    this.actualRef.instance.addEventListener('mouseup', this.mouseUpListener);
    this.actualRef.instance.addEventListener('mouseenter', this.mouseEnterListener);
    this.actualRef.instance.addEventListener('mouseleave', this.mouseLeaveListener);
  }
  ...
  public getRootElement(): HTMLElement {
    return this.actualRef.instance;
  }
  ...
  protected setPrimed(isPrimed: boolean): void {
    ...
    if (isPrimed) {
      ...
      if (this.inhibitOnDrag) {
        this.actualRef.instance.addEventListener('mousemove', this.mouseMoveListener);
      }
    } else {
      this.cssClassSet.delete('touch-button-primed');
      if (this.inhibitOnDrag) {
        this.actualRef.instance.removeEventListener('mousemove', this.mouseMoveListener);
      }
    }
  }
  ...
  protected onMouseDown(e: MouseEvent): void {
    ...
    this.action.set(this.props.onTouched ? this.props.onTouched(this) : TouchButtonOnTouchedAction.Prime);
  ...
  protected onMouseLeave(e: MouseEvent): void {
    ...
    if (this.focusOnDrag && this.actualRef.instance.parentElement) {
      const newE = new MouseEvent('mousedown', {
        clientX: e.clientX,
        clientY: e.clientY,
        bubbles: true,
      });
      this.actualRef.instance.parentElement.dispatchEvent(newE);
    }
  }
  ...
  public render(): VNode {
    ...
    return (
      <div ref={this.actualRef} class={this.cssClassSet} style={this.styles}>
        {this.renderLabel()}
        {this.props.children}
      </div>
    );
  ...
  public destroy(): void {
    ...
    this.actualRef.instance.removeEventListener('mousedown', this.mouseDownListener);
    this.actualRef.instance.removeEventListener('mouseup', this.mouseUpListener);
    this.actualRef.instance.removeEventListener('mouseenter', this.mouseEnterListener);
    this.actualRef.instance.removeEventListener('mouseleave', this.mouseLeaveListener);
    this.actualRef.instance.removeEventListener('mousemove', this.mouseMoveListener);
