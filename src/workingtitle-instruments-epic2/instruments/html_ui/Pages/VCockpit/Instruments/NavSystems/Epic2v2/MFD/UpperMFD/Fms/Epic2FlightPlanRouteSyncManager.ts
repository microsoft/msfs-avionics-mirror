...
import { Epic2FlightPlanRouteLoader } from './Epic2FlightPlanRouteLoader';
import { Epic2Fms } from '@microsoft/msfs-epic2-shared';
import { Epic2FlightPlanRouteUtils } from './Epic2FlightPlanRouteUtils';

/**
 * A manager for syncing Epic2 flight plan routes to and from the sim. The manager supports automatically replying to
 * avionics route requests and can be used to manually or automatically load synced flight plan routes.
 */
export class Epic2FlightPlanRouteSyncManager {
  private routeManager?: FlightPlanRouteManager;
  private routeLoader?: Epic2FlightPlanRouteLoader;
  ...
  /**
   * Creates a new instance of Epic2FlightPlanRouteSyncManager. The manager is created in an uninitialized state and
   ...
   */
  public init(manager: FlightPlanRouteManager, loader: Epic2FlightPlanRouteLoader): void {
    if (!this.isAlive) {
      throw new Error('Epic2FlightPlanRouteSyncManager: cannot initialize a dead manager');
    }
  ...
  public startAutoReply(): void {
    if (!this.isAlive) {
      throw new Error('Epic2FlightPlanRouteSyncManager: cannot manipulate a dead manager');
    }
  ...
  public loadFromSyncedRoute(exec: boolean): Promise<boolean> {
    ...
    return this.routeLoader!.loadRoute(syncedRoute, exec);
  }
  ...
  public loadRoute(route: ReadonlyFlightPlanRoute, exec: boolean): Promise<boolean> {
    ...
    return this.routeLoader.loadRoute(route, exec);
  }
  ...
  private onSyncedAvionicsRouteChanged(route: ReadonlyFlightPlanRoute | null): void {
    ...
    this.routeLoader!.loadRoute(route, false);
  }
    ...
    do {
      ...
      try {
        if (this.fms.hasPrimaryFlightPlan()) {
          route = await Epic2FlightPlanRouteUtils.createRouteFromFlightPlan(
            this.fms.facLoader,
